import { currentPage, darkMode, unreadCount, sidebarOpen, activeShift, mobileNavOpen } from "@/logic/state/app-state";
...
export function Header() {
  ...
  return (
    <header class="min-h-14 pt-safe bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex items-center px-3 sm:px-4 gap-3 sticky top-0 z-20">
      <button
        onClick={(e) => {
          // < lg = mode mobile: tombol membuka sheet Menu; desktop: toggle sidebar
          if (window.innerWidth < 1024) mobileNavOpen.value = true;
          else sidebarOpen.value = !sidebarOpen.value;
          (e.currentTarget as HTMLElement).blur();
        }}
        class="p-1.5 -ml-1 touch-target rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
        aria-label="Menu"
      >
        <Icons.Menu size={20} />
      </button>
