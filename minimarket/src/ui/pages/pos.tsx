...
export function POSPage() {
  ...
  return (
    <div class="flex flex-col lg:flex-row gap-0 lg:gap-0 -mx-4 -mt-4 sm:-mx-6 sm:-mt-6 lg:mx-0 lg:my-0 lg:h-[calc(100vh-3.5rem)] overflow-visible lg:overflow-hidden">

      {/* ── LEFT COLUMN: Keranjang Belanja ── */}
      <div class="flex flex-col w-full min-w-0 lg:w-[38%] bg-gray-50 dark:bg-gray-900 lg:border-r border-gray-200 dark:border-gray-700 lg:overflow-hidden">
        ...
        {/* Cart items */}
        <div class="flex-1 min-h-[120px] lg:min-h-0 overflow-y-auto scrollbar-thin">
          {items.length === 0 ? (
      ...
      {/* ── MOBILE ONLY: Grid Pilih Produk (di bawah kartu pencarian) ── */}
      <div class="lg:hidden border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
        <div class="px-3 pt-2 pb-3">
          <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">
            Pilih Produk
            {posSearchQuery.value && <span class="normal-case font-normal"> — "{posSearchQuery.value}"</span>}
          </p>
          <div class="grid grid-cols-2 sm:grid-cols-3 gap-1.5 max-h-72 overflow-y-auto scrollbar-thin">
            {productService.search(posSearchQuery.value, posCategoryFilter.value).slice(0, 30).map(p => (
              <button
                key={p.id}
                onClick={() => {
                  posService.addToCart(p.id);
                  showToast(`✓ ${p.name}`, "success", 1200);
                  if (navigator.vibrate) navigator.vibrate(10);
                }}
                disabled={p.stock <= 0}
                class="text-left p-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 active:bg-primary-50 dark:active:bg-primary-900/20 active:border-primary-300 disabled:opacity-40 transition-colors"
              >
                <p class="text-xs font-semibold text-gray-900 dark:text-gray-100 truncate">{p.name}</p>
                <p class="text-[11px] font-bold text-primary-600 dark:text-primary-400 mt-0.5">{formatRupiah(p.sellPrice)}</p>
                <p class="text-[10px] text-gray-400 mt-0.5">Stok {p.stock} {p.sellUnit}</p>
              </button>
            ))}
            {productService.search(posSearchQuery.value, posCategoryFilter.value).length === 0 && (
              <p class="col-span-full text-center text-xs text-gray-400 py-6">Tidak ada produk cocok</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col w-full min-w-0 lg:w-[30%] bg-white dark:bg-gray-800 lg:border-r border-gray-200 dark:border-gray-700 lg:overflow-hidden">
      ...
      <div class="w-full lg:w-[32%] min-w-0 bg-gray-50 dark:bg-gray-900 lg:overflow-hidden lg:h-full">
...
function KasRequiredScreen() {
  ...
  return (
    <div class="flex flex-col items-center justify-center min-h-[70vh] lg:h-full -mx-4 -mt-4 sm:-mx-6 sm:-mt-6 lg:mx-0 lg:my-0 bg-gray-50 dark:bg-gray-900">
      <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-8 sm:p-10 mx-4 max-w-md text-center border border-gray-100 dark:border-gray-700">
