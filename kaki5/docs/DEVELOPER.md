...
Versi acuan: **v194 / 1.0.119 (2026-09-09)**. Setiap klaim disertai `file:baris`.
...
<script type="module" src="js/app.js?v=184"></script>  <!-- :1014 entry -->
...
dan `updateOverlay` (force update). Satu sumber daftarnya `HARD_GATE_OVERLAYS`
(`js/modal.js:35-48`, diekspor) — dipakai listener Escape (`modal.js:53-60`),
`closeAllModals()` (`modal.js:163-165`), klik backdrop/navbar (`app.js:974,978`), dan
`navigation.js:69`. **Jangan** menambah jalur tutup dengan daftar `except:` sendiri; itu
persis celah yang membuat gerbang bisa dilewati lewat Escape sebelum v170.
**Sejak v181 `bukaKasModal` TIDAK lagi hard gate** (permintaan pemilik): gerbang kas hanya
mengunci halaman Jualan — Escape/backdrop/navigasi bisa menutupnya, dan watcher
MutationObserver di `kas.js:168-184` menegakkan ulang modal selama masih di Jualan tanpa
shift terbuka (debounce 80ms); navbar selalu di atas backdrop gerbang
(`#bukaKasModal.show` z 340 < `.bottom-nav` 350).
...
| `modal.js` | `openModal`/`closeModal`/focus trap + `registerModalSelector` (`app.js:40` memuat `tcModal`) + **`HARD_GATE_OVERLAYS:48`** — satu daftar overlay yang tidak boleh ditutup dari UI (`lockOverlay`, `updateOverlay`; `bukaKasModal` dikeluarkan sejak v181 — gerbang kas ditegakkan per-halaman oleh watcher `kas.js`) |
...
v170 juga **tidak** memuat `closeBukaKasModal` — tidak ada tombol tutup di dalam modal
(`app.js:66-69`, `case 'close-buka-kas'` dihapus dari dispatcher). Sejak v181 modal ini bisa
ditutup lewat jalur generik (Escape/backdrop/navigasi memanggil `closeModal`), tetapi
watcher `kas.js:168-184` menegakkannya kembali selama masih di halaman Jualan tanpa shift.
...
| `openBukaKasModal` / `closeBukaKasModal` / `bukaKas` | 135 / 150 / 154 | modal `#bukaKasModal` (`index.html:713`, `.modal-center` sejak v170). **`closeBukaKasModal` TIDAK di-wire ke `window`** (tidak ada tombol tutup di dalam modal); sejak v181 modal bisa ditutup via Escape/backdrop/navigasi (jalur generik `closeModal`) dan watcher `kas.js:168-184` menegakkannya kembali selama di Jualan tanpa shift; fungsi tetap diekspor & dipanggil internal `bukaKas()` setelah shift tersimpan |
...
> Dikonfirmasi ulang pada **v182 / 1.0.107** (2026-09-07): sinyal identik (2 MISSING
> `*-ojol-row` — handler-nya memang hidup di `menu.js`, bukan `app.js`; 4 DEAD case),
> `test-css-drift` kini **856 rule** dan tetap **hijau**.
...
```
                    ...
                    └───────────────┬──────────────────────────┘
                                    │ dexie.min.js (global) → app.js?v=184 (ESM)
                    ┌───────────────▼──────────────────────────┐
                    │  app.js  — entry                          │
...
*Terakhir diselaraskan dengan kode: **v194 / 1.0.119, 2026-09-09**.*
