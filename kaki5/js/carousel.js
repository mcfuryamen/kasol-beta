/* =========================================================================
   KASIR SOLO - KAKI5
   carousel.js — Platform messages carousel. Imports: db, app-state, helpers.
   ========================================================================= */
...
export async function renderPlatformCarousel() {
    ...
    msgs = [
      {
        ...
        body: 'Gunakan Kasir Solo untuk mencatat jualan & pengeluaran harian dengan mudah',
        emoji: '👋',
        order: 0,
        gradient: 'linear-gradient(135deg, #E85D04 0%, #FF8C42 100%)',
        visibleFrom: now,
        visibleUntil: null
      },
      {
        id: 'demo2',
        title: '📊 Pantau Omzet',
        body: 'Lihat statistik penjualan harian, laba, dan pengeluaran Anda secara real-time',
        emoji: '📈',
        order: 1,
        gradient: 'linear-gradient(135deg, #1982C4 0%, #2EABCA 100%)',
        visibleFrom: now,
        visibleUntil: null
      },
      {
        id: 'demo3',
        title: '🎁 Fitur Lengkap',
        body: 'Kelola menu, laporan, printer, dan lisensi - semua dalam satu aplikasi',
        emoji: '⭐',
        order: 2,
        gradient: 'linear-gradient(135deg, #70117E 0%, #B5368D 100%)',
        visibleFrom: now,
        visibleUntil: null
      }
    ];
  ...
  let slidesHtml = msgs.map((m, i) => `
    <div class="plat-carousel-slide" style="background:${m.gradient || gradients[i % gradients.length]}; border-radius:var(--radius);">
...
export function platNext() {
  ...
  setPlatCurrentSlide((platCurrentSlide + 1) % Math.max(1, count));
  platSetSlide(platCurrentSlide);
}
export function platPrev() {
  const count = document.querySelectorAll('.plat-carousel-slide').length;
  setPlatCurrentSlide((platCurrentSlide - 1 + count) % Math.max(1, count));
  platSetSlide(platCurrentSlide);
}
