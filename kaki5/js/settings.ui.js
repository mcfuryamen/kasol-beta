...
export async function loadSettings() {
  ...
  // Audit toast 2026-09-07: silent=false — kegagalan tarik profil saat user
  // membuka Pengaturan wajib dikabar (boot tetap silent via default).
  try { await pullCloudProfileIfOnline(false); } catch (e) { console.warn('[SETTINGS] pull profil gagal:', e?.message || e); }
  const data = await loadSettingsData();
  // Header baris-1 = nama usaha — sinkron juga tiap buka Pengaturan/post-pull.
  try { if (window._ksr_applyHeaderBizName) window._ksr_applyHeaderBizName(); } catch (_) {}
  ...
  // ID Perangkat kini dirender DI DALAM kartu lisensi (license.ui.js
  // licUnitHtml, 2026-09-15) — bukan lagi elemen statis di Tentang Aplikasi.
...
export async function saveOwner() {
  ...
  try {
    ...
    // Audit toast 2026-09-07: tulis DB di luar try/catch sync — gagal diam,
    // modal nyangkut, tanpa kabar.
    console.error('[SETTINGS] saveOwner gagal:', e?.message || e);
    showToast('❌ Gagal menyimpan nama pemilik — coba lagi', 'error');
    return;
  }
...
export async function saveWa() {
  ...
  try {
    ...
    // Audit toast 2026-09-07: tulis DB di luar try/catch sync — gagal diam.
    console.error('[SETTINGS] saveWa gagal:', e?.message || e);
    showToast('❌ Gagal menyimpan nomor WhatsApp — coba lagi', 'error');
    return;
  }
...
export async function saveAlamat() {
  ...
  try {
    ...
    // Audit toast 2026-09-07: 9x setSetting berurutan bisa gagal setengah jalan —
    // wajib dikabar agar user cek ulang isian.
    console.error('[SETTINGS] saveAlamat gagal:', e?.message || e);
    showToast('❌ Gagal menyimpan alamat — cek data lalu ulangi', 'error', 3500);
    return;
  }
...
export async function saveNamaUsaha() {
  ...
  try {
    ...
    // Audit toast 2026-09-07: tulis DB di luar try/catch sync — gagal diam.
    console.error('[SETTINGS] saveNamaUsaha gagal:', e?.message || e);
    showToast('❌ Gagal menyimpan nama usaha — coba lagi', 'error');
    return;
  }
