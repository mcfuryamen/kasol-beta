...
export async function refreshShiftCache() {
  try {
    ...
    setOpenShift(null);
    // Audit toast 2026-09-07: gagal baca dianggap "kas tutup" — tanpa kabar,
    // toast "Kas belum dibuka" berikutnya menyesatkan. Kabari sekali per sesi.
    if (!refreshShiftCache._warned) {
      refreshShiftCache._warned = true;
      showToast('⚠️ Status kas gagal dibaca — periksa data kas', 'error', 4000);
    }
  }
  return openShift;
}
...
// Gerbang kas terbatas halaman Jualan (v180, permintaan pemilik): dulu
// bukaKasModal HARD_GATE yang mengunci seluruh aplikasi (Escape/backdrop/
// navigasi diblok). Kini hanya Jualan — bila modal ditutup SELAGI masih di
// halaman Jualan tanpa shift terbuka, gerbang ditegakkan kembali; pindah
// halaman bebas. Pengamat memantau class .show modal supaya semua jalur tutup
// (Escape, backdrop, sweep app.js, closeAllModals navigasi) tertangkap.
let _kasGateWatcher = null;
function armKasGateWatcher() {
  if (_kasGateWatcher || typeof MutationObserver === 'undefined') return;
  const el = document.getElementById('bukaKasModal');
  if (!el) return;
  _kasGateWatcher = new MutationObserver(() => {
    if (el.classList.contains('show')) return;
    // Debounce: beri jeda agar navigasi sempat pindah halaman dulu — kalau
    // pengguna sudah di halaman lain, gerbang dibiarkan tertutup.
    setTimeout(async () => {
      if (currentPage !== 'jualan' || el.classList.contains('show')) return;
      try {
        if (await fiturKasAktif() && !(await getOpenShift())) await openBukaKasModal();
      } catch (_) { /* kondisi tak terbaca — masuk Jualan berikutnya menegakkan */ }
    }, 80);
  });
  _kasGateWatcher.observe(el, { attributes: true, attributeFilter: ['class'] });
}
armKasGateWatcher();
...
async function _bukaKasCore() {
  ...
  try {
    ...
    // Audit toast 2026-09-07: gagal tulis shift = modal nyangkut tanpa kabar.
    console.error('[KAS] kasShift.add gagal:', e?.message || e);
    showToast('❌ Gagal membuka kas — coba lagi', 'error', 3500);
    return;
  }
  closeBukaKasModal();
  await refreshShiftCache();
...
export async function tutupKas() {
  ...
  try {
    ...
    // Audit toast 2026-09-07: gagal di tengah = shift tetap "buka", modal
    // nyangkut, toast sukses di bawah tak pernah tercapai — wajib dikabar.
    console.error('[KAS] kasShift.update (tutup) gagal:', e?.message || e);
    showToast('❌ Gagal menutup kas — data belum tersimpan, coba lagi', 'error', 4000);
    return;
  }
  closeTutupKasModal();
  await refreshShiftCache();
...
export async function simpanTutupBuku() {
  ...
  showConfirm('📕', 'Tutup buku tahun ' + tahun + '? Rekap: laba ' + formatRp(r.laba) + ' dari ' + r.jumlahTransaksi + ' transaksi. Rekap tersimpan permanen.', 'Ya, Tutup Buku', async () => {
    try {
      ...
      // Audit toast 2026-09-07: dialog sudah tertutup saat callback jalan —
      // gagal simpan tanpa kabar membuat user mengira buku sudah ditutup.
      console.error('[KAS] tutupBuku.add gagal:', e?.message || e);
      showToast('❌ Gagal menyimpan rekap tutup buku — coba lagi', 'error', 4000);
      return;
    }
    closeTutupBukuModal();
