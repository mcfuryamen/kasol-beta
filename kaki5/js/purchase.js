...
export async function openPurchaseSheet() {
  ...
  const demoPayment = {
    ...
    accountNumber: '—',
    accountName: 'Atur di Control Center'
  };
  ...
  try {
    ...
    // Audit toast 2026-09-07: gagal muat info pembayaran → sheet diam-diam
    // memakai konten DEMO — user yang mau bayar tidak tahu harus coba lagi.
    console.error('Failed to load payment info', e);
    showToast('⚠️ Gagal memuat info pembayaran — cek internet lalu buka lagi 📶', 'error', 5000);
  }
  ...
  const qrisHtml = hasQris
    ...
    : `<div style="text-align:center;padding:20px;border:1px dashed var(--line,var(--border));border-radius:12px;color:var(--text2)">
         ...
         <div style="font-size:12px;margin-top:4px">QRIS asli belum diatur di Control Center.</div>
       </div>`;
  ...
  const bankHtml = (payInfo.bank || payInfo.accountNumber || payInfo.accountName)
    ...
    : `
      <div style="background:var(--bg2);border-radius:12px;padding:12px;margin-bottom:16px;font-size:13px;color:var(--text2)">
        💳 <strong>Mode demo:</strong> detail pembayaran belum dikonfigurasi. Jangan transfer sebelum admin mengisi QRIS dan rekening di Control Center.
      </div>`;
  ...
  body.innerHTML = `
    ...
    <div style="margin-top:16px;padding:12px;background:rgba(250,204,21,0.15);border-radius:8px;font-size:13px;color:var(--text2)">
      ...
      2. Transfer sesuai nominal (${escapeHtml(payInfo.priceLabel) || 'lihat info harga'})<br>
...
export async function submitPurchase(unitId, deviceCode) {
  ...
  showToast('⏳ Mengupload bukti...', 'info', 2000);
  ...
  try {
    ...
    // Anti-enumerasi (port rosok 841ca35, audit keamanan 2026-09-14): bucket
    // bukti public-read — suffix acak membuat URL tak bisa ditebak meski
    // unitId & jam upload diketahui.
    const rnd = Array.from(crypto.getRandomValues(new Uint8Array(6)), b => b.toString(36)).join('').slice(0,8);
    const fileName = `${unitId}_${timestamp}_${rnd}.jpg`;
    ...
    showToast('✅ Bukti pembayaran dikirim! Tunggu verifikasi admin.', 'success', 3000);
  ...
  const check = async () => {
    ...
    if (status?.license_status === 'aktif') {
      ...
      try {
        ...
        // Audit toast 2026-09-07: toast sukses sudah tampil tapi chip tetap
        // "TRIAL" tanpa kabar tambahan.
        console.warn('Persist lisensi lokal gagal:', e);
        showToast('⚠️ Chip status belum terbarui — muat ulang aplikasi untuk aktif penuh', 'warning', 6000);
      }
      // Reload license info
    ...
    if (attempts >= maxAttempts) {
      _pollTimer = null;
      showToast('⏳ Verifikasi masih berlangsung. Kami akan mengaktifkan lisensi Anda segera.', 'info', 5000);
      return false;
    }
...
export function subscribeToLicenseUpdates(unitId) {
      ...
      if (payload.new.license_status === 'aktif') {
        ...
        (async () => {
          try {
            ...
            // Audit toast 2026-09-07: jalur realtime — sama dengan jalur polling.
            console.warn('Persist lisensi lokal gagal:', e);
            showToast('⚠️ Chip status belum terbarui — muat ulang aplikasi untuk aktif penuh', 'warning', 6000);
          }
          if (window._ksr_updateTrialChip) window._ksr_updateTrialChip();
  ...
  channel.subscribe((status) => {
    // Audit toast 2026-09-07: error channel dulu diam total — janji "aktif
    // otomatis" bisa mati tanpa kabar (polling 30 dtk masih menolong).
    if (status === 'CHANNEL_ERROR' && !subscribeToLicenseUpdates._errWarned) {
      subscribeToLicenseUpdates._errWarned = true;
      showToast('📡 Koneksi pemantau lisensi terputus — status tetap dicek otomatis ⏳', 'warning', 5000);
    }
  });
