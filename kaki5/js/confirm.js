...
export async function showConfirm(icon, text, btnText, callback, cancelText) {
  ...
  _confirmYesHandler = () => {
    ...
    try {
      const r = cb();
      // Callback async (hapus/retur/tutup buku) yang menolak tanpa catch di
      // dalamnya = unhandled rejection diam total — tampilkan error generik
      // (callback yang ber-toast sendiri tetap menoastr lebih spesifik dulu).
      if (r && typeof r.catch === 'function') {
        r.catch(e => {
          console.error('[CONFIRM] callback async gagal:', e);
      ...
      console.error('[CONFIRM] callback gagal:', e);
      import('./helpers.js').then(({ showToast }) =>
        showToast('❌ Aksi gagal dijalankan — coba lagi', 'error', 3500)
      ).catch(() => {});
    }
