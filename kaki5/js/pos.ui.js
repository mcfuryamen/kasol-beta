...
export function selectTopping(menuId) {
  ...
  if (!item) { showToast('⚠️ Item sudah tidak ada di keranjang', 'warning', 3000); return; }
...
export function applySelectedTopping() {
  ...
  if (next[_toppingTargetMenuId]) {
    ...
    setCart(next);
    // QA 2026-09-07: dulu perubahan topping hanya di memori (setCart) tanpa
    // persist — refresh membuang hasil "Ubah Topping" diam-diam.
    saveCart();
  }
...
export function renderPOSCatAccordion(menus) {
  ...
  box.onclick = async (e) => {
    ...
    try {
      ...
      // Audit toast 2026-09-07: daftar menu tidak terfilter tanpa alasan.
      console.error('[POS] accordion cat click:', err?.message || err);
      showToast('❌ Gagal mengganti kategori — coba lagi', 'error');
    }
  };
}
...
function ensurePosCatDelegation(box) {
  ...
  box.addEventListener('click', async (e) => {
    ...
    try {
      ...
      // Audit toast 2026-09-07: sama dengan jalur accordion.
...
export async function openCartModal() {
  ...
  box.innerHTML = items.map(c => {
    ...
    // Topping tags dengan qty per-topping — v181: kontainer SENDIRI, posisinya
    // DI BAWAH baris qty/subtotal, melebar penuh & mengalir horizontal bila
    // topping banyak (lihat blok v181 di style.css).
    // Revisi 2026-09-08 (permintaan pemilik): tombol "🧂 Ubah Topping" dihapus —
    // fungsinya (buka panel Pilih Topping) dipindah ke kontainer tags ini
    // (klik tag = ubah topping tanpa menaikkan qty produk utama). Item yang
    // belum punya topping terpilih tetap dapat pintu masuk lewat placeholder.
    const toppingTags = hasToppings
      ? `<div class="cart-topping-tags" role="button" tabindex="0" data-action="select-topping" data-menu-id="${c.menu.id}" aria-label="Ubah topping ${escapeHtml(c.menu.nama)}">` +
        (selected.length > 0
          ? selected.map(t => {
          ...
          : '<span class="topping-tag topping-tag-hint">+ Pilih topping 🧂</span>')
        + '</div>'
      : '';
    // v181 (revisi): SATU baris — info menu+harga (kiri) | quantity | subtotal
    // (kanan); topping & catatan menyusul sebagai baris lebar penuh di bawahnya
    // (flex-wrap: blok width:100% otomatis pindah baris).
...
export function hitungKembalianUI(total, bayar) {
  ...
  val.textContent = (bayar >= total && bayar > 0) ? formatRp(bayar - total) : 'Rp 0';
  // Revisi 2026-09-08: kotak kembalian merah/oranye tipis — teks merah saat ada
  // kembalian, abu saat Rp 0.
  val.style.color = (bayar >= total && bayar > 0) ? 'var(--red)' : 'var(--text3)';
}
