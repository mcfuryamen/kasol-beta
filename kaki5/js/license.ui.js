...
// Baris ID Perangkat — pindah dari blok Tentang Aplikasi ke KARTU lisensi
// (permintaan pemilik 2026-09-15): di kartu aktif posisinya tepat di bawah
// serial; di kartu trial/cabut di bawah kartu. User butuh ID ini untuk minta
// bantuan, jadi tampil di SEMUA state kartu (Pengaturan + sheet Status Lisensi).
function licUnitHtml(deviceCode) {
  const code = (deviceCode || '').trim();
  // TANPA id — kartu bisa dirender di dua area sekaligus (Pengaturan + sheet
  // 'Status Lisensi'); id duplikat = HTML invalid & getElementById salah tangkap.
  return code ? `<div class="kfs11 ktext3 lic-unit">ID Perangkat: ${escapeHtml(code)}</div>` : '';
}
...
// Tombol WhatsApp & aktivasi manual DIHAPUS (keputusan pemilik 2026-09-14):
// semua proses lisensi terpusat di aplikasi Control via Supabase.
export function licenseStatusHtml(st) {
  ...
  return `
    ...
    ${licUnitHtml(st.deviceCode)}
  `;
}

// ── Cloud status cards ────────────────────────────────────────────────
function activeLicenseCardHtml(serial, expTxt, deviceCode) {
  return `
...
function revokedLicenseCardHtml(deviceCode) {
  return `
...
function pendingVerificationHtml() {
  return `
 ...
 */
export async function renderLicenseStatusArea(targetId) {
  ...
  if (cloud && cloud.license_status === 'aktif') {
      ...
      if (local?.status !== 'active') {
        ...
        if (!persisted?.valid) {
          // Audit toast 2026-09-07: kartu akan tampil "✓ Lisensi Aktif"
          // (membaca cloud) padahal chip/gate lokal tetap trial — divergensi diam.
          console.warn('[LICENSE] Persist lisensi cloud gagal:', persisted?.message || persisted);
    ...
    el.innerHTML = activeLicenseCardHtml(cloud.license_serial || lic?.serial, expTxt, lic?.deviceCode || cloud.device_code);
  ...
  if (cloud && (cloud.license_status === 'batal' || cloud.license_status === 'nonaktif')) {
    ...
    else el.innerHTML = revokedLicenseCardHtml(cloud.device_code);
    await markLicenseRevoked('admin');
    if (_updateTrialChip) _updateTrialChip();
    return false;
  }
  ...
  if (cloud && cloud.license_status === 'menunggu_verifikasi') {
    el.innerHTML = pendingVerificationHtml();
    return false;
  }

  const st = await getLicenseStatus();
  el.innerHTML = licenseStatusHtml(st);
  return false;
}
...
export async function checkCloudStatusAndUnlock() {
  ...
  const cloud = await fetchLicenseStatusFromCloud();
  if (cloud && cloud.license_status === 'aktif') {
    await renderLicenseStatusArea('licenseSheetBody');
    closeModal('lockOverlay');
    hideQuotaBanner();
    if (_checkLicenseGate) _checkLicenseGate();
    showToast('🎉 Lisensi berhasil diaktifkan!', 'success', 3000);
    return true;
  }
    if (cloud && (cloud.license_status === 'batal' || cloud.license_status === 'nonaktif')) {
      await enforceRevoked();
      showToast('Lisensi telah dinonaktifkan oleh admin.', 'error', 3000);
      return false;
    }
    showToast('Lisensi belum aktif. Pembayaran menunggu verifikasi admin.', 'info', 3000);
    return false;
  }

/**
 * Toggle input serial manual (fallback tersembunyi) DIHAPUS 2026-09-14
 * (aktivasi manual tidak lagi tersedia — terpusat di Control).
 */
...
export async function checkLicenseGate() {
  ...
  if (document.getElementById('lockLicenseStatusArea')) {
    const active = await renderLicenseStatusArea('lockLicenseStatusArea');
    if (active) {
      if (_updateTrialChip) _updateTrialChip();
      if (_renderLicenseInfoCard) _renderLicenseInfoCard();
      const lock = document.getElementById('lockOverlay');
  ...
  if (await isLicensed()) {
    ...
    card.innerHTML = `
      <div class="card license-card-active">
        ...
        ${licUnitHtml(lic.deviceCode)}
        <p class="license-expiry">${expTxt}</p>
        <p class="license-desc">Lisensi terikat perangkat ini dan membuka semua fitur tanpa batasan.</p>
      </div>`;
    return;
  }
  await renderLicenseStatusArea('licenseInfoCard');
  }

// ── Share / Extension UI ──────────────────────────────────────────────
// contactViaWA & toggleManualKey & activateLicense DIHAPUS (2026-09-14,
// keputusan pemilik): beli lisensi hanya via sheet (bukti → verifikasi
// admin) dan aktivasi terpusat di aplikasi Control via Supabase.
...
export function renderProfileMismatchOverlay() {
  ...
  if (page) {
    page.innerHTML = `
      ...
      <div class="kfs12 ktext3 kmt14">Perbaiki profil di Pengaturan, atau hubungi admin (Control Center) untuk penyesuaian lisensi.</div>
    `;
  }
  const area = document.getElementById('lockLicenseStatusArea');
  if (area) area.innerHTML = '';
