...
const _bantuanWireMap = { __wired: false, initBantuan: 'initBantuan', toggleTutorial: 'toggleTutorial', switchBantuanTab: 'switchBantuanTab' };
...
window._ksr_acceptTC = async () => {
  try {
    ...
    // Audit toast 2026-09-07: gagal simpan persetujuan = modal tetap terbuka
    // & pilihan mati tanpa kabar.
...
function _handleDataActionInner(action, el, event) {
  switch (action) {
    ...
    case 'activate-license': {
      // Audit toast 2026-09-07: dulu memanggil activateSerial (pure logic) dan
      // hasilnya DIBUANG — sukses maupun gagal sama-sama diam. Fungsi ber-toast
      // activateLicense (UI sheet) yang memang dirancang untuk tombol ini.
      if (el?.dataset.inputId) {
        activateLicense(el.dataset.inputId).catch(e => {
          console.error('[APP] activateLicense gagal:', e);
          showToast('❌ Gagal mengaktifkan kode — coba lagi', 'error');
        });
      }
      break;
    }
