...
// (2026-09-14) registerModalSelector tcModal DIHAPUS — konten S&K pindah ke
// tab Bantuan; modal baru #changelogModal memakai struktur standar .modal-overlay.
...
const _menuWireMap = { __wired: false, renderMenuList: 'renderMenuList', renderMenuListDebounced: 'renderMenuListDebounced', openMenuForm: 'openMenuForm', closeMenuModal: 'closeMenuModal', saveMenu: 'saveMenu', toggleMenu: 'toggleMenu', confirmDeleteMenu: 'confirmDeleteMenu', addCustomSuplayer: 'addCustomSuplayer', addCustomKategori: 'addCustomKategori', pickKategori: 'pickKategori', pickSuplayer: 'pickSuplayer', deleteKategoriCustom: 'deleteKategoriCustom', deleteSuplayerCustom: 'deleteSuplayerCustom', syncPakaiStokToggle: 'syncPakaiStokToggle', openReturModal: 'openReturModal', closeReturModal: 'closeReturModal', confirmRetur: 'confirmRetur', openKonsinyasiRetur: 'openKonsinyasiRetur' };
...
const _bantuanWireMap = { __wired: false, initBantuan: 'initBantuan', toggleTutorial: 'toggleTutorial', switchBantuanTab: 'switchBantuanTab', openChangelog: 'openChangelog' };
...
// Audit toast 2026-09-07: gagal lazy-wire = navigasi terbuka tapi semua tombol
// halaman itu mati tanpa penjelasan. Kabarkan sekali per sesi.
let _wireFailToastShown = false;
function _wireFailToast() {
  if (_wireFailToastShown) return;
  _wireFailToastShown = true;
  showToast('⚠️ Sebagian fitur gagal dimuat — muat ulang aplikasi', 'error', 5000);
}
...
}).catch(e => { console.error('[APP] Failed to wire pos:', e); _wireFailToast(); });
...
}).catch(e => { console.error('[APP] Failed to wire beranda:', e); _wireFailToast(); });
...
}).catch(e => { console.error('[APP] Failed to wire kas:', e); _wireFailToast(); });
...
}).catch(e => { console.error('[APP] Failed to wire menu:', e); _wireFailToast(); });
...
}).catch(e => { console.error('[APP] Failed to wire laporan:', e); _wireFailToast(); });
...
}).catch(e => { console.error('[APP] Failed to wire settings:', e); _wireFailToast(); });
...
}).catch(e => { console.error('[APP] Failed to wire bantuan:', e); _wireFailToast(); });
...
}).catch(e => { console.error('[APP] Failed to wire pengeluaran:', e); _wireFailToast(); });
...
// Header app — baris 1 = NAMA USAHA (pemilik 2026-09-14; subtitle statis
// "Kasir Solo - Kaki Lima" di markup). Sumber: settings.namaUsaha (legacy
// namaWarung ikut), fallback 'Kasir Solo' selama profil belum diisi.
// Dipanggil: boot (setelah pull), loadSettings (buka Pengaturan / refresh
// post-pull), saveNamaUsaha.
export async function applyHeaderBizName() {
  const el = document.getElementById('hdrBizName');
  if (!el) return;
  let nama = await getSetting('namaUsaha', null);
  if (nama == null) nama = await getSetting('namaWarung', null);
  nama = (nama || '').trim();
  el.textContent = nama || 'Kasir Solo';
  el.title = nama || 'Kasir Solo';
}
window._ksr_applyHeaderBizName = applyHeaderBizName;
window._ksr_toggleModal  = toggleModal;

// ==================== LICENSE GATE ====================
// Expose license actions to the gate UI (index.html)
// (activateLicense, contactViaWA, toggleManualKey dihapus 2026-09-14 —
//  aktivasi manual & jalur WhatsApp tidak lagi tersedia; terpusat di Control)
import { setLicenseRefs, updateTrialChip, renderLicenseInfoCard, checkLicenseGate, openLicenseSheet, isLicensed, getLicenseStatus, startTrial, activateSerial, checkCloudStatusAndUnlock, fetchLicenseStatusFromCloud, isDeviceKnownOnCloud, saveLicense, getDeviceIdentity, decodeExpiryLabel, enforceRevoked } from './license.js';
...
// --- Syarat & Ketentuan (2026-09-14 restruktur Bantuan, selaras rosok) ---
// T&C kini TAB di halaman Bantuan (📜 S & K) — TIDAK ada lagi modal sekali-jalan
// saat boot. Tombol "✓ Saya Setuju" di panel S&K (baris #tcAcceptRow) hanya
// tampil selama tcAcceptedAt belum tercatat; persetujuan tetap dicatat di db.
window._ksr_acceptTC = async () => {
  try {
    await setSetting('tcAcceptedAt', new Date().toISOString());
  } catch (e) {
    // Audit toast 2026-09-07: gagal simpan = tombol mati tanpa kabar.
    console.error('[APP] acceptTC gagal:', e);
    showToast('❌ Gagal menyimpan persetujuan — coba lagi', 'error');
    return;
  }
  const row = document.getElementById('tcAcceptRow');
  if (row) row.style.display = 'none';
  showToast('✅ Terima kasih — persetujuan tersimpan');
};
...
async function guardLicensedThen(fn) {
  if (await isLicensed()) {
    try { await fn(); } catch (e) {
      // Audit toast 2026-09-07: error internal backup.js sudah ber-toast; yang
      // luput sampai sini umumnya dynamic import gagal (chunk stale) — diam
      // berarti klik tombol berbayar mati tanpa kabar.
      console.warn('[guard] aksi gagal:', e);
      showToast('❌ Aksi gagal dijalankan — muat ulang aplikasi lalu coba lagi', 'error', 4000);
    }
    return;
  }
...
function handleDataAction(action, el, event) {
  // Audit toast 2026-09-07: satu throw di handler mana pun = klik mati tanpa
  // jejak di UI (tidak ada handler unhandledrejection global). Gerbang
  // try/catch tunggal di sini menutup semua case sekaligus.
  try {
    _handleDataActionInner(action, el, event);
  } catch (e) {
    console.error('[APP] data-action "' + action + '" gagal:', e);
    showToast('❌ Aksi gagal dijalankan — coba lagi', 'error', 3500);
  }
}

function _handleDataActionInner(action, el, event) {
  switch (action) {
    // Navigation
    case 'navigate-bantuan':
      ...
      break;
    case 'bantuan-tab':
      if (window.switchBantuanTab) window.switchBantuanTab(el?.dataset.tab || 'tutorial');
      break;
    ...
    case 'close-changelog':
      // CSP kaki5 memblokir onclick inline — tombol Tutup changelog WAJIB lewat
      // dispatcher data-action (bug "klik ga respon": onclick window._ksr_closeModal
      // tidak pernah terpasang; ditemukan & diperbaiki pemilik 2026-09-15).
    ...
    case 'pick-suplayer':
      ...
      break;
    case 'del-kategori-custom':
      if (window.deleteKategoriCustom) window.deleteKategoriCustom(el?.dataset.value ?? '');
      break;
    case 'del-suplayer-custom':
      if (window.deleteSuplayerCustom) window.deleteSuplayerCustom(el?.dataset.value ?? '');
      break;
    case 'retur-menu':
    ...
    case 'retry-menu-list': {
      ...
      // Audit toast 2026-09-07: satu-satunya aksi user di state error — gagal
      // retry dulu ditelan kosong.
      import('./menu.js').then(m => m.renderMenuList()).catch(() => {
        showToast('❌ Masih gagal memuat menu — coba beberapa saat lagi', 'error');
      });
      break;
    }
    case 'retry-pos': {
      import('./pos.js').then(m => m.loadPOS()).catch(() => {
        showToast('❌ Masih gagal memuat halaman Jualan — coba beberapa saat lagi', 'error');
      });
      break;
    }
    case 'open-menu-form': {
      ...
      window.showConfirm(
        ...
        () => {
          try { window.clearCart(); } catch (e) {
            // Audit toast 2026-09-07: user sudah konfirmasi — gagal kosongkan
            // tak boleh diam.
    ...
    // (case contact-via-wa / toggle-manual-key / activate-license dihapus
    //  2026-09-14 — aktivasi manual & jalur WhatsApp tidak lagi tersedia)
    case 'open-purchase-sheet':
    case 'buy-gate':
      openPurchaseSheet();
      break;
    case 'check-license-status':
      // Audit toast 2026-09-07: syncLicenseStatus() fire-and-forget — hasil
      // {ok:false, reason} tidak pernah dibaca, gagal network & sukses sama-sama
      // diam, dan kartu di sheet tidak dirender ulang. checkCloudStatusAndUnlock
      // menoastr semua cabang (aktif / belum aktif / dicabut).
      checkCloudStatusAndUnlock().catch(e => {
        console.error('[APP] cek status lisensi gagal:', e);
        showToast('⚠️ Gagal memeriksa status — cek internet lalu coba lagi 📶', 'error', 4000);
      });
      break;
...
async function init() {
  ...
  document.addEventListener('click', (e) => {
    ...
    // update) TIDAK boleh ditutup lewat klik backdrop maupun navbar, supaya
    // gate-nya gak bisa dilewati. bukaKasModal sejak v180 bukan hard gate lagi
    // (gerbang kas hanya ditegakkan di Jualan oleh kas.js).
  ...
  async function continueKnownDevice() {
      ...
      try {
        ...
        // Audit toast 2026-09-07: pemilik lisensi bisa turun ke trial senyap
        // hanya karena cek cloud gagal saat boot. Kabarkan sekali bila online.
        if (navigator.onLine && !continueKnownDevice._warned) {
          continueKnownDevice._warned = true;
          showToast('⚠️ Gagal memeriksa lisensi ke cloud — lisensi aktif belum terbaca 📶', 'warning', 6000);
        }
      }
    ...
    async function saveLicenseFromCloud(cloud) {
      try {
        ...
        // Audit toast 2026-09-07: status cloud aktif tapi gagal persist lokal —
        // gate/chip tetap membaca 'trial' tanpa kabar.
        showToast('⚠️ Lisensi belum tersimpan di perangkat — muat ulang aplikasi bila chip belum aktif', 'warning', 6000);
      }
    }
...
async function boot() {
  ...
  // Header nama usaha — setelah pull supaya nilai cloud terbaru langsung tampil.
  try { await applyHeaderBizName(); } catch (_) { /* header fallback statis aman */ }
  ...
  // Syarat & Ketentuan sekali-jalan DIHAPUS (2026-09-14, restruktur Bantuan):
  // pemicu S&K kini SATU-satunya tab 📜 S & K di halaman Bantuan — tidak ada
  // modal auto-muncul saat boot. Persetujuan dicatat via tombol di panel itu.
