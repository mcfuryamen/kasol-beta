...
export function switchTxnTab(mode) {
  ...
  // Tanggal form yang tampil default ke hari ini bila masih kosong. Dulu tanggal
  // hanya diisi lewat openExpenseForm/openIncomeForm (isiForm); pindah tab tak
  // mengisi → tab Pemasukan (incTanggal) kosong padahal Pengeluaran sudah hari ini.
  const tgl = el(f.tgl);
  if (tgl && !tgl.value) { tgl.max = todayStr(); tgl.value = todayStr(); }
  return jenis;
}
...
async function refreshSetelahSimpan() {
  try { await loadReport(); } catch (e) {
    // Audit toast 2026-09-07: toast sukses sudah tampil, tapi angka layar basi —
    // jangan biarkan user mengira angka sudah termasuk catatan baru.
    console.warn('[PENGELUARAN] refresh laporan:', e?.message || e);
    showToast('⚠️ Catatan tersimpan, tapi angka layar belum diperbarui — buka ulang Laporan', 'warning', 4000);
  }
  try {
    const kas = await import('./kas.js');
