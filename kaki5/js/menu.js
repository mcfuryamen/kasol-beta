...
export async function openMenuForm(id) {
  ...
  try {
    if (id) {
      ...
      if (!m) { showToast('⚠️ Menu tidak ditemukan — daftar disegarkan', 'warning', 3000); await renderMenuList(); return; }
...
async function renderKategoriOptions(selectedVal) {
  ...
  box.innerHTML = opts.map(v => {
    const isCustom = custom.includes(v);
    const label = isCustom ? '📦 ' + v : catLabel(v);
    // Permintaan pemilik 2026-09-08: chip custom dapat dihapus manual lewat
    // × kecil bulat di pojok kanan atas (preset bawaan tanpa tombol).
    const del = isCustom
      ? `<button type="button" class="acc-opt-del" data-action="del-kategori-custom" data-value="${escapeAttr(v)}" aria-label="Hapus kategori ${escapeHtml(v)}">×</button>`
      : '';
    const chip = `<button type="button" class="acc-opt${v === sel.value ? ' selected' : ''}" data-action="pick-kategori" data-value="${escapeAttr(v)}">${escapeHtml(label)}</button>`;
    return isCustom ? `<span class="acc-opt-wrap">${chip}${del}</span>` : chip;
...
async function renderSuplayerAcc(selVal) {
  ...
  box.innerHTML = opts.map(v => {
    ...
    const del = isCustom
      ? `<button type="button" class="acc-opt-del" data-action="del-suplayer-custom" data-value="${escapeAttr(v)}" aria-label="Hapus suplayer ${escapeHtml(v)}">×</button>`
      : '';
    const chip = `<button type="button" class="acc-opt${v === sel.value ? ' selected' : ''}" data-action="pick-suplayer" data-value="${escapeAttr(v)}">${escapeHtml(label)}</button>`;
    return isCustom ? `<span class="acc-opt-wrap">${chip}${del}</span>` : chip;
  }).join('');
  box.insertAdjacentHTML('beforeend',
...
// ── Hapus kategori/suplayer custom (× di pojok chip, 2026-09-08) ────────────
// Audit pemilik: chip custom wajib tampil setempat ia dibuat sampai DIHAPUS
// MANUAL di sini. Guard: masih dipakai menu = tolak (data menu jangan jadi
// yatim); 'Umum' & preset bawaan tidak punya tombol × dari awal.

export async function deleteKategoriCustom(value) {
  if (!value) return;
  if (_CAT_OPTIONS.includes(value)) return; // preset tak terhapus
  const dipakai = await DB.menu.where('kategori').equals(value).count();
  if (dipakai > 0) {
    showToast(`📦 Kategori "${value}" masih dipakai ${dipakai} menu — pindahkan menunya dulu`, 'error', 4500);
    return;
  }
  showConfirm('🗑️', `Hapus kategori "${value}"?`, 'Ya, Hapus', async () => {
    const custom = ((await getSetting('kategoriCustom', [])) || []).filter(v => v !== value);
    if (custom.length === ((await getSetting('kategoriCustom', [])) || []).length) return; // sudah tak ada
    try {
      await setSetting('kategoriCustom', custom);
    } catch (e) {
      console.error('[MENU] hapus kategori custom gagal:', e?.message || e);
      showToast('❌ Gagal menghapus kategori — coba lagi', 'error');
      return;
    }
    const sel = document.getElementById('menuKategori');
    if (sel && sel.value === value) sel.value = 'Makanan';
    await renderKategoriOptions();
    if (sel) await populateKategoriSelect(sel.value);
    showToast(`🗑️ Kategori "${value}" dihapus`);
  });
}

export async function deleteSuplayerCustom(value) {
  if (!value || value === 'Umum') return;
  const dipakai = await DB.menu.filter(m => (m.suplayer || 'Umum') === value).count();
  if (dipakai > 0) {
    showToast(`🤝 Suplayer "${value}" masih dipakai ${dipakai} menu — pindahkan menunya dulu`, 'error', 4500);
    return;
  }
  showConfirm('🗑️', `Hapus suplayer "${value}"?`, 'Ya, Hapus', async () => {
    const custom = ((await getSetting('suplayerCustom', [])) || []).filter(v => v !== value);
    if (custom.length === ((await getSetting('suplayerCustom', [])) || []).length) return; // sudah tak ada
    try {
      await setSetting('suplayerCustom', custom);
    } catch (e) {
      console.error('[MENU] hapus suplayer custom gagal:', e?.message || e);
      showToast('❌ Gagal menghapus suplayer — coba lagi', 'error');
      return;
    }
    const sel = document.getElementById('menuSuplayerSelect');
    if (sel && sel.value === value) sel.value = 'Umum';
    await renderSuplayerAcc();
    if (sel) await populateSuplayerSelect(sel.value);
    if (window.syncPakaiStokToggle) syncPakaiStokToggle();
    showToast(`🗑️ Suplayer "${value}" dihapus`);
  });
}
  ...
  showInputDialog({
    ...
    callback: async (name) => {
      ...
      try {
        ...
        // Audit toast 2026-09-07: dialog sudah tertutup — gagal simpan diam.
        console.error('[MENU] simpan suplayer gagal:', e?.message || e);
        showToast('❌ Gagal menyimpan suplayer — coba lagi', 'error');
        return;
      }
  ...
  showInputDialog({
    ...
    callback: async (name) => {
      ...
      try {
        ...
        // Audit toast 2026-09-07: dialog sudah tertutup — gagal simpan diam.
        console.error('[MENU] simpan kategori gagal:', e?.message || e);
...
export async function saveMenu() {
  ...
  try {
    ...
    // Audit toast 2026-09-07: sukses ber-toast, gagal tulis DB diam — modal
    // tidak tertutup, user tidak tahu menu belum tersimpan.
    console.error('[MENU] simpan menu gagal:', e?.message || e);
    showToast('❌ Gagal menyimpan menu — coba lagi', 'error', 3500);
    return;
  }
  closeMenuModal();
  renderMenuList();
...
export async function toggleMenu(id) {
  const m = await DB.menu.get(id);
  if (!m) { showToast('⚠️ Menu tidak ditemukan / sudah dihapus — daftar disegarkan', 'warning', 3000); await renderMenuList(); return; }
  try {
    await DB.menu.update(id, { aktif: m.aktif ? 0 : 1 });
  } catch (e) {
    // Audit toast 2026-09-07: gagal tulis = daftar tak berubah tanpa kabar.
    console.error('[MENU] toggle menu gagal:', e?.message || e);
...
export function confirmDeleteMenu(id) {
  showConfirm('🗑️', 'Yakin mau hapus menu ini?', 'Ya, Hapus', async () => {
    try {
      ...
      console.error('[MENU] hapus menu gagal:', e?.message || e);
...
async function confirmKonsinyasiRetur() {
  ...
  try {
    ...
    // Audit toast 2026-09-07: gagal di tengah loop = sebagian stok/utang
    // suplayer SUDAH terubah (mengaruhi hitungan setoran) — wajib dikabar.
    console.error('[MENU] retur konsinyasi gagal:', e?.message || e);
    showToast('❌ Gagal menyimpan retur — sebagian barang mungkin sudah terubah, periksa lagi', 'error', 5000);
    return;
  }
  closeReturModal();
...
export async function confirmRetur() {
  ...
  try {
    ...
    // Audit toast 2026-09-07: gagal tulis = stok/retur tak berubah diam.
    console.error('[MENU] retur gagal:', e?.message || e);
    showToast('❌ Gagal mencatat retur — coba lagi', 'error');
    return;
  }
  closeReturModal();
