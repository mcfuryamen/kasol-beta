...
// Jatah OFFLINE (2026-09-20): tambal anti-kebocoran tier gratis utk device
// yang tak pernah online (install PWA lalu offline selamanya). Modul PURE —
// hmac/storage diinjecte; lihat quota.offline.js.
import {
  quotaEnsure, quotaTickTx, quotaBudgetStatus,
  OFFLINE_DAYS_LIMIT, OFFLINE_TRX_LIMIT, OFFLINE_MONTH_LIMIT,
...
export async function getLicenseStatus() {
  ...
  if (lic.status === 'revoked') {
    ...
    if (lic.status === 'trial') {
      ...
      // Jatah OFFLINE (2026-09-20): offline >= 30 hari JAU (lifetime >= 300 trx
      // ATAU usia >= 3 bulan) → dikunci; check-in online SATU kali unblocks
      // (syncLicenseStatus set lastOnlineAt → offlineDays di-below ambang).
      try {
