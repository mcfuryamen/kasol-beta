...
// Audit toast 2026-09-07: modul "No DOM access" — toast lazy via dynamic
// import, sekali per sesi supaya tidak mengganggu tiap aksi.
const _cartToastFlags = {};
function _cartToastOnce(key, msg, type, duration) {
  if (_cartToastFlags[key]) return;
  _cartToastFlags[key] = true;
  import('./helpers.js').then(({ showToast }) => showToast(msg, type, duration)).catch(() => {});
}
...
export function saveCart() {
  try {
    ...
    // Audit toast 2026-09-07: cart tetap jalan di memori, tapi reload = isi
    // keranjang lenyap — user wajib diberi tahu penyebabnya storage.
    _cartToastOnce('persist', '⚠️ Penyimpanan penuh — keranjang bisa hilang saat aplikasi ditutup 💾', 'warning', 5000);
  }
}
...
export async function loadCart() {
  try {
    ...
    // Audit toast 2026-09-07: cart tersimpan tidak bisa dipulihkan lalu dibuang
    // diam-diam — user menyangka lupa sendiri.
    _cartToastOnce('load', '⚠️ Keranjang tersimpan rusak — tidak bisa dipulihkan 🛒', 'warning', 5000);
    setCart({});
    setResumedHeldId(null);
  }
}
...
export async function payHeldSync(id, { items, totalHarga, totalModal, orderType, orderNote, ojolPlatform, bayar, kembalian, metodeBayar, buktiBayar, catatanBayar }) {
  ...
  // Opsi A (audit pemilik 2026-09-08): pembayaran held BESOKNYA harus masuk
  // hari pembayaran — Laporan & Beranda memfilter per `tanggal`, bukan paidAt.
  // Dulu tanggal & waktu tetap milik hari penahanan sehingga omzet hari ini
  // tidak terlihat padahal uang sudah masuk (kas sendiri sudah pakai paidAt).
  // Nomor TRX sengaja dipertahankan dari saat tahan (jejak audit).
  const now = Date.now();
  const patch = {
    status: 'completed',
    tanggal: todayStr(),
