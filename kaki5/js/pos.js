...
export async function resumeHeldOrder(heldId, opts = {}) {
  ...
  const perform = async () => {
    ...
    if (clampedItems.length) {
      showToast(`${clampedItems[0]}${clampedItems.length > 1 ? ' +' + (clampedItems.length - 1) + ' lainnya' : ''} stok kurang — jumlah disesuaikan 🛒`, 'warning', 4000);
    }
...
export async function addToCart(menuId) {
  ...
  if (!m) { showToast('⚠️ Menu sudah dihapus — daftar disegarkan', 'warning', 3000); renderPOSMenu(); return; }
  ...
  if (toppings.length > 0 || hasOjol) {
    openMenuSelector(m, ({ selectedToppings = [], orderType: tipe = orderType, qty = 1, selectedToppingQtys = null, itemNote = '' }) => {
      ...
      // Audit toast 2026-09-07: addToCartLogic bisa MENOLAK (stok habis/qty
      // melebihi stok) dengan mengembalikan cart yang sama — dulu modal
      // tertutup & item tak masuk tanpa pesan (bandingkan changeQty).
      if (next === cart) {
        showToast(`"${m.nama}" stok tidak cukup (sisa ${m.stok || 0}) 📦`, 'error', 3000);
        return;
      }
      setCart(next);
...
export async function simpanPenjualan(cetakJuga = false) {
  ...
  try {
    ...
    // Audit toast 2026-09-07: checkout = jalur uang; gagal tulis DB (storage
    // penuh, private mode, DB terkunci) tidak boleh diam total.
    console.error('[POS] simpanPenjualan gagal:', e?.message || e);
    showToast('❌ Gagal menyimpan transaksi — cek penyimpanan lalu coba lagi', 'error', 4000);
  } finally {
...
async function _simpanPenjualanCore(cetakJuga, items) {
  ...
  if (licSt.status === 'expired') {
    if (licSt.offlineBudget) {
      // Jatah offline habis (tier gratis yang tak pernah online): bukan waktu
      // beli — butuh internet SATU KALI buat verifikasi & lanjut.
      showToast('Jatah offline habis — hubungkan internet SEKALI untuk verifikasi & lanjut jualan 📶', 'error', 5500);
      import('./license.js').then(m => m.openLicenseSheet?.()).catch(() => {});
      return;
    }
  ...
  if (resumedId) {
    try {
      ...
      // Audit toast 2026-09-07: fallback ini berubah hasil — row "ditahan"
      // asli tetap menggantung & nomor TRX baru; user wajib tahu.
      console.warn('[POS] payHeldSync gagal, simpan sebagai penjualan baru:', e?.message || e);
      saleId = null;
      showToast('⚠️ Pesanan ditahan gagal disambung — tersimpan sebagai transaksi baru; cek daftar Ditahan 🤚', 'warning', 5000);
    }
  }
