...
import { getUnitId, getDeviceCode, getInstallId, getDeviceIdentity, getLicense, saveLicense, markLicenseRevoked, bumpClockAnchor, currentTxMonth, cloudProfileMatchesLocal, hmacSignature } from './license.logic.js';
...
// Audit toast 2026-09-07: modul ini "NO DOM operations" — toast dijalankan
// lazy via dynamic import hanya di titik yang wajib mengabarkan user.
// Batasi frekuensi: polling 60 dtk bisa memicu event yang sama berulang.
const _licenseToastFlags = {};
async function _toastOnce(key, msg, type, duration) {
  if (_licenseToastFlags[key]) return;
  _licenseToastFlags[key] = Date.now();
  try {
    const { showToast } = await import('./helpers.js');
    showToast(msg, type, duration);
  } catch (_) { /* helpers gagal — jangan ganggu alur sync */ }
}
...
export async function reanchorUnitId({ force = false } = {}) {
  ...
  try {
    ...
    // getDeviceIdentity() — BUKAN baca mentah settings: ini menjalankan
    // konvergensi legacy sekali (kode pra-V5 → kandidat V5) SEBELUM kanonik
    // dihitung. Kalau baca mentah, kanonik memakai kode lama yang belum
    // dimigrasi dan re-anchor diam-diam jadi no-op ('already').
  ...
  if (lastAnchor && lastAnchor.to && lastAnchor.to !== canonical && !force) {
    return { ok: false, reason: 'oscillation-guard' };
  }
  ...
  if (existing) {
    ...
    if (adopt) {
      ...
      // hapus-baris) → seed SEKALI dengan profil lokal via push force: kalau
      ...
      if (!String(existing.nama_usaha || '').trim() && !String(existing.no_whatsapp || '').trim()) {
        try {
          const { ensureSynced } = await import('./sync.js');
          await ensureSynced({ force: true, silent: true });
        } catch (_) { /* seed gagal — bisa diulang dari Pengaturan → Cek Data Online */ }
      }
    ...
    console.warn('[REANCHOR] ' + canonical + ' sudah dipakai baris dengan profil TIDAK SAMA (beda: '
      + (diff.join(', ') || 'tidak terdeteksi') + ') — unit lama DIPERTAHANKAN. Selaraskan Nama Usaha / No. WA lalu jalankan Pengaturan → Diagnosa.');
  ...
  if (String(migErr.code || '') === '23505' || /duplicate key/i.test(String(migErr.message || ''))) {
    ...
    await setSetting(REANCHOR_BLOCK_KEY, memo);
    console.warn('[REANCHOR] ' + canonical + ' baru saja diklaim browser lain — unit lama DIPERTAHANKAN; coba lagi dari Pengaturan → Diagnosa.');
...
export async function syncLicenseStatus() {
  ...
  // Jatah OFFLINE (2026-09-20): bicarakan sukses dengan server = check-in — 
  // lastOnlineAt segar → budget offline otomatis ter-barsen (persis alur)
  // untuk device yang benar-benar online.
  try { await quotaMarkOnline(getSetting, setSetting, hmacSignature, deviceCode, Date.now()); } catch (_) { /* non-kritikel */ }
  ...
  try {
    ...
    if (local.status === 'trial') {
      ...
      // control/instance lain (jam perangkat beda → toleransi 5 detik).
  ...
  if (status === 'belum' || status === '') {
    ...
    if (local.status === 'active') {
      ...
      // Audit toast 2026-09-07: lisensi AKTIF diturunkan paksa ke tier gratis —
      // chip tiba-tiba "GRATIS" tanpa penjelasan.
      _toastOnce('downgrade', 'ℹ️ Lisensi kembali ke tier gratis oleh server — hubungi admin bila keliru 💬', 'warning', 6000);
    }
  ...
  if (status === 'aktif' && cloud.license_serial) {
    ...
    if (!(await cloudProfileMatchesLocal(cloud))) {
      ...
      // Audit toast 2026-09-07: user berbayar stuck "GRATIS" tanpa satu petunjuk.
      _toastOnce('adopsi-ditolak', '⚠️ Lisensi cloud belum bisa diaktifkan di perangkat ini — profil beda. Hubungi admin 💬', 'warning', 6000);
    } else {
  ...
  // Normalisasi deviceCode record lisensi (2026-09-06): kode tercemat saat
  // aktivasi bisa basi setelah konvergensi identitas legacy (kasus Chrome —
  // lisensi terlanjur aktif sebelum kodenya konvergen). Samakan dengan kode
  // perangkat aktif supaya record & diagnostik tidak menyesatkan.
  try {
  ...
  } catch (_) { /* normalisasi gagal — tampilan tetap benar via getLicenseStatus */ }
