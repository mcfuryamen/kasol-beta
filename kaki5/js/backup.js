...
async function buildBackupPayload() {
  // v5 (2026-09-08, permintaan pemilik): kategori & suplayer custom yang
  // ditambahkan manual (settings 'kategoriCustom'/'suplayerCustom') WAJIB ikut
  // kebawa backup/restore — dulu settings tidak dibawa sehingga chip custom
  // hilang setelah pulihkan. Yang dibawa HANYA dua key itu (bukan seluruh
  // settings) — profil usaha tetap sumber kebenarannya Supabase.
  const settingsRows = await DB.settings.toArray();
  const customKeys = new Set(['kategoriCustom', 'suplayerCustom']);
  const settings = settingsRows.filter(r => customKeys.has(r.key));
  return {
    version: 5,
...
async function applyBackupData(data) {
  ...
  // v5: kategori & suplayer custom ikut kebawa (file < v5 tidak punya kunci
  // settings → dianggap kosong, perilaku lama utuh).
  const settingsCustom = Array.isArray(data.settings)
    ? data.settings.filter(r => r && (r.key === 'kategoriCustom' || r.key === 'suplayerCustom'))
    : [];
  ...
  await DB.transaction(
    'rw',
    [DB.menu, DB.penjualan, DB.pengeluaran, DB.kasShift, DB.tutupBuku, DB.settings],
    async () => {
      await DB.menu.clear();
      ...
      // Restore custom lists: put() per key (bukan clear tabel) — key lain di
      // settings lokal tidak tersentuh.
      for (const row of settingsCustom) {
        if (Array.isArray(row.value)) await DB.settings.put({ key: row.key, value: row.value });
      }
    }
  );
  clearCartStorage();
...
export async function exportData() {
  ...
  try {
    ...
    // (crypto.subtle — gagal di konteks non-secure; tanpa guard ini tidak
    // ada file DAN tidak ada kabar, audit toast 2026-09-07).
    ...
    console.error('[BACKUP] export file gagal:', e?.message || e);
    showToast('❌ Gagal membuat file cadangan — coba lagi', 'error', 4000);
  }
}
...
export async function validateBackup(data) {
  ...
  // settings/pengaturan: key wajib string terisi. v5: settings hanya membawa
  // daftar custom — key lain di luar daftar ini ditolak (payload tidak boleh
  // menyelundupkan settings profil/lisensi lewat file cadangan).
  for (const r of (data.settings || [])) {
    if (!isStr(r.key) || !r.key) return 'File ditolak: ada pengaturan dengan key rusak.';
    if (data.version >= 5 && r.key !== 'kategoriCustom' && r.key !== 'suplayerCustom') {
      return 'File ditolak: ada pengaturan di luar daftar kategori/suplayer custom.';
    }
  }
...
export async function cloudSaveBackup() {
  ...
  try {
    ...
    // Housekeeping: sisakan CLOUD_KEEP versi terbaru per unit. Gagal di sini
    // BUKAN kegagalan simpan — jangan tampilkan toast "Gagal" yang menyesatkan
    // (audit toast 2026-09-07).
    try {
      ...
      console.warn('[BACKUP] housekeeping versi lama gagal (cadangan tetap aman):', e?.message || e);
      showToast('⚠️ Cadangan tersimpan — pembersihan versi lama gagal', 'warning', 4000);
    }
...
export function confirmClearAll() {
  ...
  showConfirm('⚠️', 'SEMUA data usaha akan dihapus dan tidak bisa dikembalikan! Status lisensi perangkat tetap tersimpan. Yakin?', 'Ya, Hapus Semua', async () => {
    try {
      ...
      // Audit toast 2026-09-07: 10x clear berurutan — gagal di tengah berarti
      // data setengah terhapus; diam di sini sangat menyesatkan.
      console.error('[BACKUP] clearAll gagal di tengah:', e?.message || e);
      showToast('❌ Gagal menghapus data — sebagian mungkin masih ada. Coba lagi.', 'error', 6000);
      return;
    }
    setCart({});
    clearCartStorage();
