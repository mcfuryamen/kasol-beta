...
export async function connectBTPrinter() {
  try {
    ...
    if (err.name === 'NotFoundError') {
      ...
      showToast('❌ Gagal terhubung: ' + String(err?.message || err).slice(0, 80), 'error');
    }
    clearPrinterState();
...
async function sendToPrinter(data) {
  ...
  while (attempts < MAX_ATTEMPTS) {
    ...
    try {
      ...
      let disconnectedMidway = false;
      for (let i = 0; i < bytes.length; i += chunkSize) {
        // H2: cek lagi saat looping (mungkin disconnect tengah jalan).
        // Audit toast 2026-09-07: dulu break senyap lalu return true — toast
        // "✅ Nota berhasil dicetak!" padahal nota TERPOTONG.
        if (!btCharacteristic) { disconnectedMidway = true; break; }
      ...
      if (disconnectedMidway) {
        showToast('⚠️ Printer terputus saat mencetak — nota belum lengkap', 'error', 4500);
        return false;
      }
      return true;
    } catch (err) {
...
export async function printNota() {
  ...
  // Audit toast 2026-09-07: dulu return diam (printLastNota ber-toast untuk
  // kasus identik).
  if (!id) { showToast('⚠️ Belum ada transaksi yang dipilih', 'warning', 3000); return; }
  ...
  if (printWindow && printWindow.document) {
    ...
    setTimeout(() => {
      try { printWindow.focus(); printWindow.print(); } catch (e) {
        // Audit toast 2026-09-07: gagal popup dulu cuma console.warn.
        console.warn('print popup gagal:', e);
        showToast('❌ Gagal mencetak nota — coba lagi', 'error');
      }
    }, 300);
    return;
  }
