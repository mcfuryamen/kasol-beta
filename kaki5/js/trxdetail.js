...
export async function showTrxDetail(id) {
  setSelectedTrxId(id);
  // Audit toast 2026-09-07: fungsi tanpa try/catch sama sekali — tap baris
  // transaksi saat DB gagal/record hilang = tidak ada modal, tidak ada pesan.
  try {
    const s = await DB.penjualan.get(id);
    if (!s) { showToast('⚠️ Transaksi sudah tidak ada', 'warning', 3000); return; }
    await _renderTrxDetail(s);
  } catch (e) {
    console.error('[TRXDETAIL] showTrxDetail gagal:', e?.message || e);
    showToast('❌ Gagal membuka detail transaksi — coba lagi', 'error');
  }
}

async function _renderTrxDetail(s) {
...
export async function hapusPenjualan() {
  ...
  showConfirm('🗑️', pesan, 'Ya, Hapus', async () => {
    try {
      ...
      console.error('[TRXDETAIL] hapus gagal:', e?.message || e);
