...
// Restruktur 2026-09-14 (selaras rosok): 3 tab — 📖 Tutorial | 🎬 Video | 📜 S & K.
// Changelog jadi modal (#changelogModal, pemicu baris "Versi …" Tentang Aplikasi);
// T&C pindah ke tab S&K; video tutorial bersumber dari cloud (products.tutorials,
// dikelola lewat kartu Katalog di Control Center).
...
const TUTORIALS = [
  {
    ...
    content: `
      <div class="klh18 kfs14">
        ...
        <ol class="kpl20 kmy8">
          <li>Buka aplikasinya — langsung bisa dipakai; baca <b>Syarat &amp; Ketentuan</b> kapan saja lewat tab <b>"📜 S &amp; K"</b> di halaman Bantuan ini, lalu tekan <b>"✓ Saya Setuju"</b> di sana (sekali saja)</li>
    ...
    content: `
      <div class="klh18 kfs14">
        ...
        <p class="kpl20 kmy8">
          ...
          Gerbang ini hanya mengunci halaman <b>Jualan</b> — kamu boleh menutupnya dan melihat halaman lain,
          tapi begitu kembali ke Jualan tanpa shift, modal muncul lagi.
    ...
    content: `
      <div class="klh18 kfs14">
        ...
        <ol class="kpl20 kmy8">
          ...
          <li>Barang yang sudah ada di keranjang tapi stoknya tinggal sedikit → menaikkan jumlah di atas sisa stok ditolak dengan <b>"Qty melebihi stok (N) — perbarui stok menu dulu"</b>; menekan Bayar saat jumlah melebihi stok juga diblokir</li>
        </ol>
  ...
  },
  {
    ...
    content: `
      <div class="klh18 kfs14">
        ...
        <ul class="kpl20 kmy8">
          <li>Setiap baris menampilkan <b>nama menu + harga satuan</b> di kiri, tombol <b>− jumlah +</b> di tengah, dan <b>total</b> baris itu di kanan; <b>info topping</b> (kalau ada) tampil di baris sendiri di bawahnya, melebar penuh. <b>Total Harga</b> dihitung ulang otomatis</li>
  ...
  },
  {
    ...
    content: `
      <div class="klh18 kfs14">
        ...
        <ul class="kpl20 kmy8">
          <li>Ringkasan: <b>💰 Omzet</b>, <b>🧮 Modal Bahan</b>, <b>💸 Biaya Usaha</b>, <b>📈 Untung Bersih</b>, <b>🛒 Transaksi</b>, <b>🍽️ Porsi Terjual</b>, <b>🤝 Utang Titipan</b>, <b>🛵 Ojol</b></li>
          <li><b>🤝 Utang Titipan</b> = total saldo terutang ke penitip (sisa positif per suplayer, dihitung sejak awal — tidak berubah saat filter tanggal diganti). Catatan non-usaha (setor bank / modal tambahan) tetap terbaca di blok <b>"💰 Rincian Pemasukan"</b></li>
    ...
    content: `
      <div class="klh18 kfs14">
        ...
        <ol class="kpl20 kmy8">
          ...
          <li>Tekan <b>"Hubungkan Printer"</b> lalu pilih printer thermal kamu — statusnya berubah jadi <b>"✅ Terhubung: (nama printer)"</b></li>
  ...
  },
  {
    ...
    content: `
      <div class="klh18 kfs14">
        ...
        <p class="kpl20 kmy8">
          Tekan <b>"Diagnosa"</b> di kartu <b>📱 Perangkat</b> → modal <b>"🩺 Diagnosa"</b> memeriksa
    ...
    content: `
      <div class="klh18 kfs14">
        ...
        <p class="kpl20 kmy8">
          ...
          di modal <b>"🩺 Diagnosa"</b>, dan di layar kunci lisensi.
...
export function initBantuan() {
  ...
  renderVideos();
  refreshTcAcceptState();
  loadCloudVideos(); // fire-and-forget; render ulang saat data tiba
}

// ==================== VIDEO TUTORIAL (tab 🎬, 2026-09-14) ====================
// Arsitektur: pemilik menempelkan URL/iframe embed YouTube per aplikasi lewat
// kartu Katalog di Control Center → tersimpan di kolom cloud `products.tutorials`
// (jsonb array). Klien menariknya lewat getSupabaseClient() (pola fetchProductSalt).
// Format item: string URL polos ATAU objek {judul?, url, durasi?, deskripsi?}.
// VIDEOS lokal = fallback offline/dev semata (isi kosong → empty-state rapi).
const VIDEOS = [];
...
async function loadCloudVideos() {
  ...
  } catch (_) { /* cloud video list bersifat opsional — silent */ }
  ...
  return `
    <div class="card kmb12">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
        ...
        <div class="kfw700 kfs16 kflex-1">${judul}</div>
        ${item.durasi ? `<span class="kfs11 ktext3">${escapeHtml(String(item.durasi))}</span>` : ''}
      </div>
      ${item.deskripsi ? `<div class="kfs13 ktext2" style="margin-bottom:10px">${escapeHtml(String(item.deskripsi))}</div>` : ''}
...
function renderVideos() {
  ...
  box.innerHTML = list || `
    <div class="card"><div class="kfs14 ktext2 klh16">🎬 Video tutorial akan segera hadir. Sementara ini, panduan lengkap tersedia di tab <b>📖 Tutorial</b>.</div></div>`;
}

// Baris "✓ Saya Setuju" pada tab S&K — tampil hanya bila belum pernah disetujui.
function refreshTcAcceptState() {
  const row = document.getElementById('tcAcceptRow');
  if (!row) return;
  getSetting('tcAcceptedAt', null).then((v) => { row.style.display = v ? 'none' : 'block'; }).catch(() => { row.style.display = 'block'; });
}

// ==================== CHANGELOG (modal — pemicu baris "Versi …") ====================
// PERAWATAN: setiap rilis baru (bump), tambahkan entri di PALING ATAS — versi
// paling atas = terbaru. Bahasa singkat, tanpa jargon teknis.
const CHANGELOGS = [
  {
    versi: '1.0.136',
    items: [
      'Perbaikan tampilan: banner status kuota kini tampil rapi — sebelumnya saat kuota habis, tombol di banner keluar dari layar.',
      'Perbaikan tampilan: saat kartu penyambutan (lengkapi profil) tampil, chip GRATIS & tombol Bantuan di header tetap bisa diakses.',
      'Penguatan internal: modul jatah offline kini ikut dijadwalkan saat pemasangan, agar aplikasi tetap nyala setelah pembaruan walau tanpa internet.'
    ]
  },
  {
    versi: '1.0.135',
  ...
  },
  {
    versi: '1.0.133',
    items: [
      'Nama usaha di header kini menyinkronkan otomatis saat tarik profil di background — tanpa perlu buka Pengaturan lagi.',
      'Perbaikan dalam: sanitasi HTML satu-lapisan, docs sinkron kode.'
    ]
  },
  {
    versi: '1.0.132',
    items: [
      'Form Pemasukan langsung terisi tanggal hari ini saat kamu pindah tab dari Pengeluaran — tidak perlu mengisi tanggal manual lagi.'
    ]
  },
  {
    versi: '1.0.131',
    items: [
      'Tombol "Pasang Aplikasi" kini menampilkan proses (Memasang… → Menginstal… → Sudah Terpasang) dengan spinner.',
      'Label "Cek Data Online" berganti menjadi "Diagnosa".',
  ...
  },
  {
    versi: '1.0.130',
    items: [
      'Ikon home diperbesar — logo Kasir Solo kini mengisi layar utama dengan lebih tegas.',
      'Ikon tab (favicon) memakai logo bundar asli.'
    ]
  },
  {
    versi: '1.0.129',
    items: [
      'Tombol "OKE" overlay update kini responsif — layar pembuka otomatis diperbarui di belakang layar, OKE hanya mengaktifkan lalu memuat ulang tanpa jeda lama.'
    ]
  },
  {
    versi: '1.0.128',
    items: [
      'Layar pembuka bawaan perangkat (native Chrome) kini jadi satu-satunya splash — lebih ringan dan langsung menyatu dengan ikon full-bleed.'
    ]
  },
  {
    versi: '1.0.127',
  ...
  },
  {
    ...
    items: [
      ...
      'Tombol "Pasang Aplikasi" kini mendeteksi status dengan benar: aktif saat belum dipasang atau habis dicopot, pasif saat sudah terpasang.'
    ]
  },
  {
    versi: '1.0.125',
    items: [
      'Pemberitahuan update kini tampil ala Rosok: latar gradasi oranye penuh, logo bulat di kartu, tombol OKE oranye.',
      'Perbaikan: tombol "Tutup" pada jendela riwayat versi yang sebelumnya tidak merespon klik, kini berfungsi normal dan berwarna oranye.',
      'Ikon aplikasi di-bake ulang (gradasi oranye + logo bulat) — layar pembuka aplikasi kini menampilkan tampilan baru.'
    ]
  },
  {
    versi: '1.0.124',
  ...
  },
  {
    versi: '1.0.123',
    items: [
      'Halaman Bantuan kini 3 tab: 📖 Tutorial | 🎬 Video | 📜 S & K — syarat & ketentuan pindah ke tabnya sendiri dan dibersihkan dari teks basi.',
      'Riwayat versi pindah tempat: ketuk baris "Versi …" di Pengaturan → Tentang Aplikasi.',
      'Judul header sekarang menampilkan nama usahamu; ID Perangkat tampil tepat di bawah nomor seri lisensi.',
      'Hero Bantuan jadi gradasi oranye senada header; tab 🎬 Video siap menampilkan video yang admin publikasikan lewat Control Center.'
    ]
  },
  {
    versi: '1.0.122',
    items: [
      'Pembaruan keamanan: alamat berkas bukti pembayaran lisensi di server kini ikut diacak sehingga tidak bisa ditebak dari luar.',
      'Perketat sisi server: tabel produk hanya-baca lewat aplikasi dan data internal lisensi tidak lagi terbuka di jalur publik — tanpa perubahan cara pakai.'
    ]
  },
  {
    versi: '1.0.121',
    items: [
      'Halaman Bantuan: membuka topik tutorial sekarang otomatis menggulir layar sampai judul topik pas di bawah bar tab — isi tutorial tidak pernah lagi terlewat di luar layar.',
      'Perbaikan: pindah tab Tutorial | Changelog kini selalu mulai dari puncak daftar (sebelumnya posisi gulir lama ikut terbawa).'
    ]
  },
  {
    versi: '1.0.120',
    items: [
      'Pembelian lisensi kini satu jalur lewat halaman Beli Lisensi (QRIS/transfer + kirim bukti) dan diverifikasi admin di Control Center — kolom kode manual dan tombol WhatsApp dihapus dari semua layar.',
      'Info pembayaran di halaman Beli Lisensi lebih aman (data rekening & harga disanitasi dari nilai server).'
    ]
  },
  {
    versi: '1.0.119',
    items: [
      'Sebutan ekosistem diselaraskan — halaman katalog kini bernama "Shop" konsisten di seluruh aplikasi Kasir Solo. Tanpa perubahan fitur.'
    ]
  },
  {
    versi: '1.0.118',
    items: [
      'Dashboard kami kini bernama "Control Center" — istilah di layar beli lisensi ikut disesuaikan. Tidak ada perubahan cara pakai.'
    ]
  },
  {
    versi: '1.0.117',
    items: [
      'Laporan custom: rentang terbalik ditolak dengan pesan; grafik rentang panjang (2 bulan ke atas) kini per bulan agar terbaca.'
    ]
  },
  {
    versi: '1.0.116',
    items: [
      'Kalender custom: kedua panah selalu tampil — kalender atas maksimal bulan berjalan, kalender bawah minimal satu bulan setelahnya.'
    ]
  },
  {
    versi: '1.0.115',
    items: [
      'Filter Laporan disempurnakan: tanggal custom dipilih dua klik (klik 1 = mulai, klik 2 = akhir merah), panah geser bulan di tiap kalender, rentang tidak valid ditolak.',
      'Pesanan ditahan yang dibayar besoknya kini tercatat di hari pembayaran.'
    ]
  },
  {
    versi: '1.0.114',
    items: [
      'Pesanan ditahan yang dibayar besoknya sekarang tercatat di hari pembayaran (omzet & laporan hari itu) — nomor nota tetap memakai nomor saat ditahan.'
    ]
  },
  {
    versi: '1.0.113',
    items: [
      'Baris keranjang lebih padat; nama menu tidak lagi terpotong (bisa membungkus 2 baris).',
      'Uang Diterima & Kembalian: HP kecil 1 kolom, HP normal ke atas 2 kolom. Kotak uang diterima hijau, kembalian merah saat ada nilainya.'
    ]
  },
  {
    versi: '1.0.112',
    items: [
      'Pembaruan aplikasi kini benar-benar lewat tombol OKE: overlay selalu tampil sampai diklik, baru aplikasi dimuat ulang ke versi terbaru.'
    ]
  },
  {
    versi: '1.0.111',
    items: [
      'Overlay pembaruan bertahan meski aplikasi di-refresh.',
      'Tambah topping tanpa menambah jumlah produk: klik info topping di baris keranjang.',
      'Kategori & suplayer custom ikut terselamatkan lewat cadangan/pemulihan + tombol × di chip untuk hapus manual.'
    ]
  },
  {
    versi: '1.0.110',
    items: [
      'Hardening notifikasi: jalur yang gagal di alur uang & data kini selalu ada kabar — checkout, buka/tutup kas, retur, menu, profil, cadangan.'
    ]
  },
  {
    versi: '1.0.109',
    items: [
      'Modul Bantuan diselaraskan dengan tampilan & label aplikasi terbaru.'
    ]
  },
  {
    versi: '1.0.108',
    items: [
      'Legenda grafik pindah ke bawah batang pada semua periode.'
    ]
  }
];
...
function renderChangelog() {
  ...
  if (!box) return;
  // APP_VERSION di-set app.js dari version.js — versi yang terpasang ditandai
  // chip "TERPASANG".
  ...
  box.innerHTML = CHANGELOGS.map((entry) => `
    <div class="card kmb12">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
        <span class="kfs20">🕓</span>
        <div class="kfw700 kfs16 kflex-1">Versi ${escapeHtml(entry.versi)}</div>
        ${entry.versi === current ? '<span style="background:var(--green-bg);color:var(--green);font-size:11px;font-weight:700;padding:3px 8px;border-radius:8px;white-space:nowrap">TERPASANG</span>' : ''}
      </div>
      <ul class="kpl20 kmy8 kfs14 klh16" style="margin:0">
        ${entry.items.map(i => `<li style="margin-bottom:6px">${escapeHtml(i)}</li>`).join('')}
      </ul>
    </div>
  `).join('');
}

// Buka modal riwayat versi — dipicu dari baris "Versi …" di Tentang Aplikasi
// (dispatcher app.js case 'open-changelog'; terpanggil via window.openChangelog
// yang dipasang wire map app.js).
export async function openChangelog() {
  renderChangelog();
  const m = await import('./modal.js');
  m.openModal('changelogModal');
}

// Switch tab Tutorial | Video | S & K (tab sticky atas halaman).
export function switchBantuanTab(tab) {
  ...
  // Scroll-port halaman = .main-content — window.scrollTo di sini SELALU no-op
  // (bug laten sejak modul lahir; pola sama yg memicu auto-scroll akordeon
  // `ad6c1dc`). Reset gulir pindah tab harus lewat port yang sama; window
  // hanya fallback bila struktur berubah.
  const port = document.querySelector('.main-content');
  if (port) port.scrollTo({ top: 0, behavior: 'smooth' });
...
export function toggleTutorial(tutId) {
  ...
  // bar sticky (port dari rosok 6a0536a — di kaki5 scroll-port-nya .main-content,
  // BUKAN window; .bantuan-tabs sticky di dalam port yang sama). Sudah di
  // posisi = diam. Tutup → tidak menggulir (user memang sedang melihatnya).
  if (!isOpen) {
    requestAnimationFrame(() => {
      const head = panel.previousElementSibling;
      const tabs = document.querySelector('.bantuan-tabs');
      const port = document.querySelector('.main-content');
      if (!head || !tabs || !port) return;
      const delta = head.getBoundingClientRect().top - tabs.getBoundingClientRect().bottom - 6;
      if (Math.abs(delta) > 2) port.scrollBy({ top: delta, behavior: 'smooth' });
    });
  }
}
