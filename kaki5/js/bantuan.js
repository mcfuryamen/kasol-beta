...
// ==================== CHANGELOG (tab kanan, 2026-09-08) ====================
// Riwayat versi untuk tab Changelog. PERAWATAN: setiap rilis baru (bump),
// tambahkan entri di PALING ATAS — versi paling atas = terbaru. Bahasa
// singkat, tanpa jargon teknis.
const CHANGELOGS = [
  {
    versi: '1.0.117',
    items: [
...
function renderChangelog() {
  const box = document.getElementById('bantuanPanelChangelog');
  if (!box) return;
...
// Switch tab Tutorial / Changelog (tab sticky atas halaman).
export function switchBantuanTab(tab) {
  const isTutorial = tab !== 'changelog';
  const showEl = (id, show) => { const el = document.getElementById(id); if (el) el.style.display = show ? 'block' : 'none'; };
  showEl('bantuanPanelTutorial', isTutorial);
  showEl('bantuanPanelChangelog', !isTutorial);
  const t = document.getElementById('bantuanTabTutorial');
  const c = document.getElementById('bantuanTabChangelog');
  if (t) { t.classList.toggle('active', isTutorial); t.setAttribute('aria-selected', isTutorial ? 'true' : 'false'); }
  if (c) { c.classList.toggle('active', !isTutorial); c.setAttribute('aria-selected', isTutorial ? 'false' : 'true'); }
