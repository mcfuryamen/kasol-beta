...
export async function pullCloudProfileTo(cloudClient) {
  ...
  // N3 (audit 2026-09-17): header baris-1 = nama usaha (app.js applyHeaderBizName)
  // — refresh TUNGGAH saat profil ter-pull di background (retry loop,
  // visibilitychange, buka Pengaturan), bukan hanya boot/saveNamaUsaha.
  // Idempotent: 2 getSetting; guard internal bila #hdrBizName belum ada.
  try { if (typeof window._ksr_applyHeaderBizName === 'function') window._ksr_applyHeaderBizName(); } catch (_) {}
}
