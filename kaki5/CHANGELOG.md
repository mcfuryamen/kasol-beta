...
## 2026-09-17 (v209 / 1.0.134: perbaiki loop overlay update berkali-kali)
- **Fix loop "OKE → reload → overlay muncul lagi, berkali-kali"** (laporan pemilik):
  tombol OKE kini reload hanya bila service worker baru benar-benar aktif —
  sebelumnya reload dijalankan walau SW baru belum siap (grace 3 dtk terlalu
  pendek utk install precache), halaman dimuat versi lama lagi, dan overlay
  men-arming ulang tanpa henti (`4529cd0`).
...
## 2026-09-17 (v208 / 1.0.133: sinkron nama usaha header + perbaikan internal)
- **Nama usaha di header** kini ter-sinkronkan otomatis saat profil ter-pull di
  background (retry loop / kembali online / buka Pengaturan) — bukan hanya saat
  boot & saveNamaUsaha (hasil audit total 2026-09-17, N3) (`37d043c`).
- **Sanitasi HTML satu-lapisan**: duplikasi lokal `escapeHtml`/`buildSafeHtml` di
  `helpers.js` dihapus — satu-satunya definisi `helpers.pure.js` via star-export
  (hasil audit Info2); 39 exports helpers.js tetap, harness 45/45.
- Docs README sinkron kode: `?v=170`→`v208`, `card-hero-blue`→`card-hero` (v198),
  "aktivasi serial"→otomatis via Control (v195), catatan T&C tab S&K, alur boot
  line-num terbaru (hasil audit N1) + peringatan bump stamp S&K (N5).

## 2026-09-17 (v207 / 1.0.132: tanggal form Pemasukan default hari ini)
- Form Pemasukan (`incTanggal`) kini terisi tanggal hari ini saat pindah tab dari
  Pengeluaran. Sebelumnya `isiForm` hanya mengisi tanggal lewat
  `openExpenseForm`/`openIncomeForm`, sementara `switchTxnTab` cuma men-toggle
  visibilitas → tab Pemasukan kosong padahal Pengeluaran sudah hari ini.
  Fix: `switchTxnTab` mengisi `tgl.value`/`tgl.max` HANYA bila kosong (prefill
  saat ubah-catatan tidak tertimpa) (`93eb984`).

## 2026-09-16 (v206 / 1.0.131: feedback pasang aplikasi + label Diagnosa + posisi baris versi)
- Tombol "Pasang Aplikasi": mesin status `idle → 📲 Memasang… → 📥 Menginstal… → ✅ Sudah
  Terpasang` (spinner + anti tap-ganda), tak lagi lompat optimistik — tunggu `appinstalled`
  dengan fallback 8 dtk (`f075b42`).
- Label "Cek Data Online" → **"Diagnosa"** (item, judul modal, bantuan, pesan error) (`d5d640d`).
- Baris "Versi … riwayat versi" dipindah ke bawah "Kasir Solo - Kaki Lima", di atas
  "PT Mesin Kasir Solo", lalu dirapatkan (mt 2px) dengan jarak lebih lapang ke PT (mb 10px,
  posisi PT tetap) (`afcc1c2` + `b9c4da6`).

## 2026-09-15 (v205 / 1.0.130: ikon home diperbesar + favicon logo asli)
- Ikon home `icon-48..512.png`: persegi `#D6501C` + logo bundar asli diperbesar
  ke 88% (sebelumnya ~58%, terasa kecil) (`2af10cb`).
- Favicon `<link rel="icon">` diset ke `icon.png` (logo bundar asli, transparan)
  — link ikon persegi berukuran dihapus agar tab selalu tampil bundar.
- `apple-touch-icon` & manifest ikut ikon home baru.

## 2026-09-15 (v204 / 1.0.129: perbaiki tombol OKE overlay update — responsif tanpa jeda 6 dtik)
- Decouple instalasi SW (background saat boot) dari aktivasi (saat OKE diklik):
  `performForceUpdate` tidak lagi `reg.update()` + tunggu 6 dtk; cukup aktifkan
  `reg.waiting` (bila ada) lalu reload (`347dad0`).
- `startUpdateWatcher` memicu `reg.update()` di background SEGERA saat `pending`
  dipulihkan (tidak nunggu timer 3 dtk) — saat OKE diklik, SW biasanya sudah
  `waiting` → aktivasi cepat.
- Feedback instan di tombol OKE: disable + teks "Memperbarui…" saat diklik.
- Grace poll 3 dtk (bukan 6 dtk) untuk edge case OKE diklik sebelum install
  background selesai; `controllerchange` fallback 2 dtk.

## 2026-09-15 (v203 / 1.0.128: hapus boot splash in-app — cukup satu splash native Chrome)
- Hapus layer `#bootSplash` in-app dari HTML, CSS, dan JS — cukup satu splash
  native Chrome, lebih ringan tanpa layer ganda (`8b25eeb`).

## 2026-09-15 (v202 / 1.0.127: ikon full-bleed final — splash bebas kartu putih)
- Semua ikon 48–512 jadi kanvas OPAQUE penuh warna == `background_color`
  (#D6501C) + logo bulat + cincin (`7c0bd2f`). Alasan sama dgn rosok v81:
  Chrome splash menampilkan ikon tanpa mask + transparansi dikomposit putih —
  full-blee (standar ikon adaptive) = satu-satunya yang rapi di semua jalur.

## 2026-09-15 (v201 / 1.0.126: boot splash in-app gradasi + ikon bulat + tombol pasang deteksi state)
- `#bootSplash` in-app: gradasi oranye full-screen + logo bulat, min-tampil
  0,9 dtk, hide saat boot selesai + safety 3,5 dtk (`4c8d70a`).
- Ikon manifest 48–512 DIKEMBALIKAN ke bulat transparan (bake persegi =
  penyebab "kotak" di splash Chrome).
- Tombol "Pasang Aplikasi": `beforeinstallprompt` membersihkan flag
  `kasirsolo:pwa-installed` (bug: tak pernah terhapus → membeku "Sudah
  Terpasang" pasca-copot); `updateInstallRow` dipanggil dari bip/appinstalled/
  mq-change; pasif = opacity .45 + pointer-events none + aria-disabled.

## 2026-09-15 (v200 / 1.0.125: overlay visual ala rosok + fix tombol Tutup changelog + ikon ter-bake)
- Overlay update mengadopsi visual rosok: backdrop gradasi oranye penuh (buang
  gelap+blur), logo bulat 64px, judul 'Ada Versi Baru! 🔄', versi teks polos
  brand, catatan list disc, tombol btn-primary, kartu 400px (`f150cf7`).
- FIX tombol "Tutup" #changelogModal mati sejak lahir — CSP `script-src 'self'`
  memblokir onclick inline; pindah ke data-action close-changelog via dispatcher
  + btn-primary oranye (`65fea6a`).
- Semua ikon manifest 48–512 di-bake art splash baru (gradasi + logo bulat) —
  Chrome splash PWA = background_color + icon, bukan splash_pages (`68fa02c`).
- Slot-5 CHANGELOGS entri 1.0.125.

## 2026-09-15 (v199 / 1.0.124: splash PWA baru + aturan overlay OKE-only)
- Splash layar pembuka: 1028² gradasi oranye brand + logo BULAT jernih dari
  master icon.png 1254² (tanpa scale-up icon-512 yang bikin buram per catatan
  v182) + cincin putih; manifest `splash_pages` BARU + `background_color`
  #FFFAF5→#D6501C; sw precache += splash (`86bb70c`).
- Overlay update: aturan final pemilik — HANYA klik OKE yang menyembunyikan,
  bahkan saat build lokal sudah konvergen; pending bertahan lintas refresh &
  offline; force-update tunggu claim SW; ACK dihapus; OKE gagal mendarat →
  re-arm otomatis (`8154289` + `f1847e8`).
- Slot-5 CHANGELOGS entri 1.0.124.

## 2026-09-15 (v198 / 1.0.123: restruktur Bantuan 3 tab + pipeline video Control + header nama usaha + S&K + ID-in-kartu + hero oranye)
- Bantuan jadi 3 tab 📖 Tutorial | 🎬 Video | 📜 S & K; Changelog pindah ke modal
  dipicu baris "Versi …" Tentang Aplikasi; T&C pindah ke tabnya + "✓ Saya Setuju"
  persist; gerbang S&K auto-muncul boot DIHAPUS. Hero → gradasi oranye brand.
- Header: baris-1 = nama usaha dinamis (sinkron boot/pull/simpan), subtitle
  "Kasir Solo - Kaki Lima"; ID Perangkat pindah ke kartu lisensi bawah serial
  (semua state; tanpa id duplikat).
- Pipeline video: kolom cloud `products.tutorials` (diedit via Control → Katalog;
  sanitizer parseYoutubeId host-whitelist + iframe-code support; CSP += frame-src).
- S&K dibersihkan dari teks basi (trial/Qris lama), stamp v1.2 → v1.2 14 Sep.
- Rangkaian commit: `aa6e198` `af151eb` `049a890` `753028c` `c5da7b4` `075312d` `f1d09ff`.
- Slot-5 CHANGELOGS entri 1.0.123 di bantuan.js. QA: dua origin 8086/127 terverifikasi.

## 2026-09-14 (v197 / 1.0.122: port hasil audit keamanan rosok — fileName bukti + dokumen hardening)
- Audit menyeluruh terhadap pola kerentanan rosok (XSS renderer, skema URL, salt,
  signature): **renderer kaki5 terverifikasi bersih total** — `escapeHtml` kaki5
  entity-map lengkap (termasuk kutip), semua interpolasi user-data escaped di
  titik render/pembangun, toast & confirm textContent, app-link regex https.
- `purchase.js`: nama file bukti +suffix acak crypto 8 karakter (bucket `bukti`
  public-read — URL tak bisa ditebak meski unitId & jam upload diketahui). Port
  rosok `841ca35`.
- README/DESIGN/AGENTS diselaraskan dengan hardening cloud yang sudah dieksekusi
  2026-09-14 (kolom `products.salt` tak lagi terbaca anon; nilai cloud saat revoke
  identik dengan konstanta fallback → nol perubahan perilaku; signature backup =
  checksum integritas; peringatan rotasi salt dua langkah).
- Cloud = grant DB shared → kaki5 ikut terlindungi tanpa perubahan kode lain.
- Slot-5 `CHANGELOGS` bantuan.js entri 1.0.122.

## 2026-09-14 (v196 / 1.0.121: auto-scroll akordeon Bantuan + fix reset gulir pindah tab)
- `toggleTutorial`: saat topik dibuka, `.main-content` (scroll-port kaki5 — BUKAN
  window) auto-`scrollBy` menyelaraskan header akordeon ±6px di bawah `.bantuan-tabs`
  sticky (`rAF`; |delta|≤2px = diam; menutup panel tidak menggulir). Port dari
  rosok `6a0536a` (di rosok port-nya window, di kaki5 adaptasi kontainer).
- `switchBantuanTab` FIX bug laten sejak modul lahir (2026-09-08): `window.scrollTo`
  selalu no-op terhadap `.main-content` → pindah Tutorial|Changelog tidak pernah
  reset gulir. Kini `port.scrollTo({top:0,behavior:'smooth'})` + fallback window.
- Slot-5 `CHANGELOGS` bantuan.js: entri 1.0.121 BARU di paling atas + BACKFILL
  1.0.118–1.0.120 yang terlewat di bump-bump sebelumnya (utang perawatan sejak
  v193; chip TERPASANG tetap mengikuti APP_VERSION).
- QA IAB 8086 dua ronde 8/8: delta 6px persis, no-scroll-on-close, reset dua arah
  (640→0), panel terbuka tak tertelan reset, regresi auto-scroll lolos, onerror 0.

## 2026-09-14 (v195 / 1.0.120: lisensi terpusat di Control Center)
- Kolom "Sudah punya kode? Aktivasi manual" dan tombol "💬 WhatsApp"
  DIHAPUS dari kartu lisensi (trial/aktif/pending/revoked), sheet lisensi,
  halaman "Lisensi Dicabut", dan overlay profil-tidak-cocok. Proses lisensi
  satu jalur: Beli Lisensi (QRIS/transfer + kirim bukti) → verifikasi admin
  di Control Center.
- Sheet beli lisensi: harga di teks "Cara Pembayaran" kini di-escape
  (kelanjutan audit M6).
- Internal: `renderLicenseStatusArea` tanpa param inputId; handler
  dispatcher `activate-license`/`toggle-manual-key`/`contact-via-wa` dan
  window export-nya dihapus; CSS yatim (.btn-wa, .btn-buy-wa,
  .manual-key-*) dibersihkan.

## 2026-09-09 (v194 / 1.0.119: sinkron rename aplikasi ekosistem — Shop)
- Internal: folder `landing/` menjadi `shop/` (aplikasi katalog/beli) —
  dokumentasi ekosistem & daftar aplikasi tersinkron. Tanpa perubahan
  runtime/fitur; bump murni menyertakan rilis ekosistem.

## 2026-09-09 (v193 / 1.0.118: rebrand sebutan dashboard jadi Control Center)
- Istilah "Admin Console" pada layar Beli Lisensi (placeholder rekening,
  preview QRIS demo, banner mode demo) kini "Control Center" — selaras
  rebrand aplikasi dashboard admin jadi Control Center Kasir Solo.
- Tanpa perubahan perilaku/logika; komentar modul app-link ikut diselaraskan.

## 2026-09-07 (v184 / 1.0.109: modul bantuan diselaraskan dengan kode nyata)
- Audit penuh `js/bantuan.js` (17 tutorial) terhadap label/toast/struktur UI
  aktual; 5 klaim basi diperbaiki:
  1. Laporan: daftar KPI tidak lagi menyebut "💵 Pemasukan Usaha" &
     "🏧 Non-Usaha (laci)" (keduanya dihapus/diganti v181) — kini
     "🤝 Utang Titipan" + catatan non-usaha di blok Rincian Pemasukan.
  2. Stok: toast "… habis — hapus dari keranjang dulu 🛒" tidak ada di kode —
     diganti perilaku nyata: "Qty melebihi stok (N) — perbarui stok menu dulu"
     (pos.js:502,528) + blokir Bayar saat qty melebihi stok (M2).
  3. Printer: status "✅ … (tersimpan)" → "✅ Terhubung: (nama printer)"
     (printer.js:123).
  4. Keranjang: deskripsi baris item ditambah info topping di baris sendiri
     di bawahnya (tata letak v181).
  5. Buka & Tutup Kas: ditambahkan catatan gerbang hanya mengunci halaman
     Jualan — boleh ditutup & pindah halaman, muncul lagi saat kembali
     (perilaku v181).
- Sisanya terverifikasi cocok: label form menu/modal, kategori pengeluaran &
  pemasukan, metode bayar + bukti foto, alur tahan pesanan, chip & sheet
  lisensi, alur beli lisensi, cadangan lokal/cloud, profil + alamat 4 tingkat,
  format ID Perangkat & nomor lisensi, instalasi PWA, tutup buku tahunan,
  detail transaksi, nomor TRX/MSK/BLJ, WA admin.
- QA render di IAB localhost:8086: kelima perbaikan terverifikasi tampil.

## 2026-09-07 (v183 / 1.0.108: legenda grafik pindah ke bawah batang — semua periode)
- Permintaan pemilik: legenda "Omzet / Pengeluaran" dipindah ke BAWAH batang grafik,
  berlaku di filter Harian, Mingguan, Bulanan, dan Custom. Grafik Harian
  (`renderHourlyChart`) sudah bawah sejak 2026-09-04 (komentar browser #3);
  `renderChart` (Mingguan/Bulanan/Custom) yang masih di atas kini disamakan —
  blok legenda dipindah setelah `.chart-bars` (`margin-top:10px`, rata tengah).
  QA IAB localhost: legenda muncul di bawah batang (legendTop 883 > barsBottom
  873 Bulanan; 889 > 879 Mingguan).

## 2026-09-07 (v182 / 1.0.107: fix ikon splash PWA — icon-192/512 korup)
- Permintaan pemilik: logo saat loading awal PWA dibuka tampak buram/pecah.
  Akar: `assets/icon-192.png` & `assets/icon-512.png` (generasi 17 Agu, batch
  berbeda dari ikon lain 19 Agu) punya **CRC chunk IHDR salah** — browser
  menolak chunk CRC rusak → kedua ikon gagal decode → splash Android fallback
  ke ikon kecil (144/152) yang di-scale up → buram. Terkonfirmasi file yang
  disajikan LIVE pun rusak (1 chunk CRC bad).
- Fix: regenerate kedua ikon dari sumber tajam `assets/icon.png` (1254²) —
  resize LANCZOS ke 80% kanvas + padding transparan terpusat (resep sama
  dengan batch sehat, aman utk purpose "maskable"), CRC tervalidasi 0 bad.

## 2026-09-07 (v181 / 1.0.106: poles UI — gerbang Buka Kas, KPI Laporan, keranjang, tombol aksi detail)
- Permintaan pemilik: modal "🔓 Buka Kas" tidak lagi HARD_GATE yang mengunci
  seluruh aplikasi. Dikeluarkan dari HARD_GATE_OVERLAYS — Escape/klik
  backdrop/klik navbar/navigasi kini bisa menutupnya; pindah halaman bebas.
- Gerbang tetap ditegakkan DI HALAMAN JUALAN: watcher (MutationObserver di
  kas.js) menegakkan ulang modal selama masih di Jualan tanpa shift terbuka
  (debounce 80ms agar navigasi sempat pindah halaman). Masuk Jualan tanpa
  shift tetap memunculkan gerbang (pos.js render), dan "Bayar" tanpa shift
  tetap diblok + toast (jaring pengaman pos.js).
- CSS: `#bukaKasModal.show` z-index 340 — DI BAWAH `.bottom-nav` (350)
  supaya bottom navbar tetap bisa diklik langsung saat gerbang aktif
  (revisi: tanpa ini backdrop menutupi navbar dan klik navbar hanya
  menutup-buka modal — navbar terasa masih terkunci).
- Laporan: kartu "💵 Pemasukan Usaha" DIHAPUS — nilainya sudah tergabung di
  kartu Omzet (omzet + pemasukan usaha) sejak v178; rincian tetap terbaca di
  blok "Rincian Pemasukan".
- Laporan: kartu "🏧 Non-Usaha (laci)" menjadi "🤝 Utang Titipan" (gradasi
  ungu) — nilainya total saldo konsinyasi terutang (sisa positif per
  suplayer, dihitung sejak awal). Kartu "🛵 Ojol" jadi gradasi hijau.
  Agregat konsinyasi diekstrak ke `hitungKonsinyasiStats()` yang dipanggil
  sebelum grid KPI — satu sumber untuk kartu Utang Titipan dan blok
  Konsinyasi.
- Modal detail Pengeluaran & Pemasukan: urutan tombol aksi ditukar —
  "🗑️ Hapus" dulu dengan lebar auto ikut label, "✏️ Ubah Catatan" mengisi
  sisa area; container flex horizontal di semua perangkat (kgrid-2col-gap8
  dihindari karena media query tertentu menumpuknya jadi 1 kolom).
- Modal detail Transaksi: pola tombol aksi yang sama — "🗑️ Hapus" dulu
  (auto-size ikut label), "🖨️ Cetak Nota" mengisi sisa area; flex horizontal
  di semua perangkat (kelas .detail-actions di index.html + style.css).
- Keranjang (Jualan): item SATU BARIS — info menu+harga (kiri) | quantity
  (tengah) | subtotal (kanan); info topping pindah ke kontainer sendiri DI
  BAWAH baris itu, melebar penuh dan mengalir horizontal bila topping banyak
  (wrapper .cart-qty-price & .cart-info dihapus; blok v181 di akhir
  style.css menimpa definisi cart lama).

## 2026-09-06 (v180 / 1.0.105: rilis konvergensi identitas legacy)
- Sisi lain fix V5 yang lolos: freeze deviceCode ikut MEMBEKUKAN kode lama era
  berbeda (V3/V4/FNV via http-IP-LAN) yang tersimpan per browser → divergensi
  antar-browser yang dulu terkonvergensi jadi tersimen ("dulu bisa, sekarang
  beda lagi"). Kini instalasi TANPA penanda `fpVersion` konvergen SEKALI ke
  kandidat V5 (deterministik & hash seragam http/https) lalu membeku
  (`26dc3dc`); re-anchor membaca identitas via `getDeviceIdentity()` supaya
  kanonik selalu memakai kode yang sudah dimigrasi.
- Serial-bound ikut migrasi bila kode tercemat dalam serial = kandidat
  (`f001aa2` — kasus nyata: Chrome terlanjur mengadopsi lisensi sebelum kode
  konvergen).
- Cek "terjangkar" derive(stored.fingerprint)===deviceCode menetralkan
  penanda fpVersion basi (dulu ditulis boot yang gagal migrasi) dan
  fingerprint tersimpan dipertahankan sebagai pasangan sah deviceCode —
  freeze tidak bocor lintas boot (`f5e57d3`).
- ID Perangkat tampil dari identitas aktif (bukan `lic.deviceCode` beku saat
  aktivasi) + normalisasi record lisensi saat sync (`dc86b7b`).
- Adopsi baris kanonik yang KOSONG profilnya men-SEED profil lokal via push
  force — anti-wipe C2v2 (pull cloud-truth selainnya mengosongkan profil
  pengadopsi).

## 2026-09-06 (v179 / 1.0.104: identitas perangkat V5 — deviceCode beku sekali lahir)
- Kasus lapangan "satu perangkat terdeteksi dua deviceCode" dituntaskan: sinyal
  layar di fingerprint jadi pasangan terurut max×min (kebal rotasi), fallback
  hash FNV diganti SHA-256 murni JS (konteks http vs https tidak lagi melahirkan
  kode berbeda), deviceCode BEKU sekali lahir (nilai tersimpan menang atas
  re-derive; fingerprint segar hanya diagnostik), `navigator.storage.persist()`
  mencegah eviksi IndexedDB yang melahirkan unitId ulang, pagar anti-osilasi di
  re-anchor (konvergensi ke kanonik berbeda hanya via Cek Data Online), dan
  serial era V3+V4 tetap diterima sebagai masa tenggang (`kaki5 f7d2755`).
- Fix crash backfill sync 'write': destructure null saat baris sudah ada &
  non-force — 135 entri sync_errors sejak v171, retry tanpa henti tiap ~2,5
  menit (`kaki5 0f46268`).
- Pemulihan insiden: baris `K5-00GC-QFOT` dihapus pemilik → perangkat terkunci;
  baris dibuat ulang status 'belum' di cloud sehingga jalur H3 memulihkannya
  ke tier gratis otomatis. Pelajaran: hapus baris clients = revoke, bukan
  redirect identitas.
