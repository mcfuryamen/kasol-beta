...
export async function openCartModal() {
  ...
  box.innerHTML = items.map(c => {
    ...
    // Topping tags dengan qty per-topping. Sengaja TANPA margin-left inline:
    // v167 tag ini jadi baris sendiri selebar kartu (lihat blok "Cart v167" di
    // css/style.css), dan inline style akan mengalahkan aturan CSS tersebut.
    const toppingTags = hasToppings && selected.length > 0
      ? '<div class="cart-topping-tags">' + selected.map(t => {
      ...
      : '';
    // v167 rev.2 — KERANJANG 2 KOLOM (permintaan pemilik 2026-09-04):
    //   KOLOM KIRI (.cart-main) : baris 1 nama menu + harga menu (sebaris),
    //                            baris 2 tag topping, baris 3 catatan menu
    //   KOLOM KANAN (.cart-side): quantity di ATAS, subtotal di BAWAH
    // Kelas .cart-price dipertahankan: refreshCartModalTotals() mencari
    // subtotal per baris lewat row.querySelector('.cart-price').
    return `<div class="cart-item" data-menu-id="${c.menu.id}">
      <div class="cart-main">
        <div class="cart-name">${escapeHtml(c.menu.nama)}` +
        ...
        ` <span class="cart-name-price" title="Harga satuan">${displayHargaTipe}</span></div>
        ${toppingTags}
