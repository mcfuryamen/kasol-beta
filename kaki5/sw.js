...
// Cache version v211 — ubah angka INI juga setiap swap (harus sama dengan
...
// v192: setCustomDate validasi-dulu (rentang terbalik ditolak, state tak
// tercemar) + grafik custom rentang panjang bucket per-bulan.
// v191: panah kalender custom selalu tampil + batas arah (atas maks bulan
// berjalan, bawah min atas+1). v190: audit filter laporan + picker custom baru (klik-2x, panah bulan,
// sel akhir merah, validasi rentang terbalik). v189: bayar pesanan ditahan (tanggal &
// waktu mengikuti saat bayar, nomor TRX tetap dari saat tahan).
// v188: rapatkan baris keranjang + nama menu 2 baris; blok bayar responsif
// (HP kecil 1 kolom) + kotak uang diterima hijau / kembalian merah tipis.
// v187: update gate OKE ditegakkan — sw.js tanpa skipWaiting, aset cache-first
// (+ ignoreSearch & precache cache:reload), OKE = SKIP_WAITING + reload.
// v186: overlay update persist sampai OKE; tambah topping via klik tag info;
// kategori/suplayer custom: tombol x hapus + ikut backup-restore (v5 payload).
// v185: hardening feedback toast (audit 2026-09-07) — jalur gagal alur uang/data
// selalu dikabar (checkout, kas, retur, menu, profil, cadangan), wiring tombol
// lisensi diperbaiki (Aktifkan Kode & Refresh Status), toast di atas sheet lisensi.
// v184: modul Bantuan diselaraskan dengan kode nyata kaki5 (audit 17 tutorial)
//       terhadap label/toast/struktur UI aktual; 5 klaim basi diperbaiki (KPI
//       Laporan pasca v181, toast stok v175, status printer, baris keranjang
//       v181, perilaku gerbang kas v181).
// v183: legenda grafik Laporan pindah ke BAWAH batang pada semua periode
//       (renderChart mingguan/bulanan/custom disamakan dengan renderHourlyChart
//       yang sudah bawah sejak 2026-09-04).
// v182: fix ikon splash PWA — icon-192.png & icon-512.png punya CRC chunk
//       IHDR korup (header ditulis ulang tanpa hitung CRC) sehingga browser
//       menolak decode dan splash fallback ke ikon kecil di-scale up = buram;
//       keduanya diregenerate dari assets/icon.png (1254²) dengan padding
//       80% kanvas (resep batch ikon sehat, aman purpose "maskable").
// v181: poles UI — gerbang Buka Kas hanya mengunci halaman Jualan (navbar
//       tetap bisa diklik, z di bawah nav); KPI Laporan: "Non-Usaha" jadi
//       "🤝 Utang Titipan" (ungu, saldo terutang via hitungKonsinyasiStats),
//       Ojol hijau, kartu "Pemasukan Usaha" dihapus (sudah di Omzet);
//       keranjang satu baris (menu+harga | qty | subtotal, topping kontainer
//       sendiri di bawah); tombol aksi modal detail transaksi/pemasukan/
//       pengeluaran: Hapus dulu auto-size, aksi utama isi sisa, flex
//       horizontal semua perangkat.
// v180: konvergensi identitas legacy — instalasi pra-V5 otomatis menyesuaikan
//       kode ke kandidat V5 sekali lalu beku (kasus "kode perangkat beda
//       antar browser"); serial-bound ikut bila kode tercemat serial =
//       kandidat; cek "terjangkar" menetralkan penanda fpVersion basi;
//       fingerprint tersimpan dipertahankan sebagai pasangan sah deviceCode;
//       ID Perangkat tampil dari identitas aktif (bukan nilai beku record
//       lisensi) + normalisasi record saat sync.
// v179: identitas perangkat V5 — deviceCode beku sekali lahir (kasus satu
//       perangkat dua deviceCode): sinyal layar jadi pasangan terurut max×min
//       (kebal rotasi), fallback hash SHA-256 murni JS (FNV dihapus — http vs
//       https tidak lagi melahirkan kode berbeda), navigator.storage.persist()
//       mencegah eviksi IndexedDB, pagar anti-osilasi re-anchor, serial era
//       V3+V4 tetap masa tenggang. Plus fix crash backfill sync 'write'
//       (destructure null saat baris sudah ada & non-force — 135 entri
//       sync_errors sejak v171, retry tanpa henti tiap ~2,5 menit).
...
//       (2) Saklar Pengaturan disinkronkan dari DB di AWAL loadSettings(),
//           sebelum `pullCloudProfileIfOnline()` yang tanpa timeout.
//       (3) `saveFiturKas()` membandingkan dengan nilai TERSIMPAN, bukan cache,
//           supaya klik saklar tidak membatalkan penulisannya sendiri.
//       Gejala lama: status kas tertutup + Pengaturan mengaku aktif, tapi kios
//       bisa jualan tanpa modal awal.
...
const CACHE_NAME = 'kasir-solo-kaki5-v211';
...
// v187 (audit 2026-09-08): TANPA skipWaiting — SW baru masuk status "waiting"
// dan TIDAK aktif sampai user klik OKE di overlay update (performForceUpdate
// mengirim pesan SKIP_WAITING). Dulu skipWaiting membuat app self-update
// diam-diam: SW baru langsung claim + aset ter-fetch segar, jadi pada buka
// berikutnya lokal == server dan overlay update TAK PERNAH muncul — melanggar
// aturan pemilik "overlay wajib tampil sampai OKE diklik".
self.addEventListener('install', event => {
  event.waitUntil(
    caches.open(CACHE_NAME).then(cache => {
      // cache:'reload' → precache selalu ambil LANGSUNG dari network (bypass
      // HTTP cache) supaya cache versi baru tidak berisi salinan basi.
      const reqs = ASSETS_TO_CACHE.map(u => new Request(u, { cache: 'reload' }));
...
// Tombol OKE di overlay update yang memicu aktifasi SW yang menunggu.
...
self.addEventListener('fetch', event => {
  ...
  // v187 (audit 2026-09-08): dulu network-first — begitu ada deploy, modul JS
  // (termasuk version.js/CACHE_BUST) ter-fetch segar tanpa OKE sehingga
  // version.js == version.json dan overlay update TAK PERNAH muncul (app
  // self-update diam-diam). Sekarang cache-first: app lama tetap berjalan
  // sampai user klik OKE; versi baru berlaku hanya lewat aktifasi SW yang
  // dipicu tombol OKE (SKIP_WAITING) + reload. Miss (aset non-precache) →
  // isi runtime cache untuk kunjungan offline berikutnya.
  event.respondWith(
    caches.match(request, { ignoreSearch: true }).then(cached => {
      // ignoreSearch: precache menyimpan URL tanpa '?v=NNN', halaman meminta
      // dengan query — tanpa ini app.js?v=186 MISS -> fetch fresh = self-update.
