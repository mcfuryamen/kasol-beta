...
```
kasol/
├── control/                      # Control Center Kasir Solo (Supabase-connected, PWA)
...
├── shop/                       # Landing Page Marketing (eks-landing)
...
3. **Push perubahan di `shop/`** → project Vercel Shop deploy otomatis (project lain skip)
4. **dst.** untuk `shop/`, `control/`, `gerobak/`, `retail/`, `landing-new/`
...
> **Catatan control:** `control/` memakai satu build command kecil (`scripts/build-env-loader.mjs`) yang menulis `js/env-loader.js` dari **Vercel environment variables** saat deploy — key Supabase **tidak pernah di-commit**. Env diisi lewat konektor **Vercel + Supabase**.
...
- `kasirsolo.com` → Shop
- `ctrl.kasirsolo.com` → Dashboard Control
...
**Aplikasi:** rosok, gerobak, retail, kaki5, shop, control
