# 📊 Control Center Kasir Solo

Dashboard internal untuk mengelola seluruh ekosistem KASIRSOLO — leads, katalog aplikasi, lisensi, dan pengaturan shoping page.
...
#     bukan dari dalam control/. Env dibaca dari .env di root repo.)
...
- **Shop config:** Hero title/desc/CTA
...
|-----|--------|---------|---------|
| `kasirsolo:catalog` | Katalog aplikasi | Control Center | Shop |
| `kasirsolo:settings` | Pengaturan shop | Control Center | Shop |
| `kasirsolo:leads` | Data leads | Shop form | Control Center |
| `kasirsolo:stats` | Analytics | Shop | Control Center |
...
# 3. Vercel auto-deploy (project: control, root: control/)
```
...
**Tujuan:** testing Control Center dengan data Supabase staging (kasol-beta). Deploy otomatis setiap push ke branch `kasol-beta`.
...
| File | Isi |
|------|-----|
| [`docs/00-ekosistem.md`](docs/00-ekosistem.md) | Peta ekosistem KASIRSOLO (Control Center, shop, apps, Supabase) |
...
- ✅ Outreach `outreach_contacts` (2026-09-18) — kampanye reaktivasi klien lama:
  impor CSV (dedupe per nomor, prefix 628 saja), status alur
  belum→terkirim→bales→minat→beli (+mati/optout), catatan, KPI strip,
  ekspor CSV. RLS tanpa policy publik (service role saja); whitelist tabel
  di `server.js` + `api/rest.js`.
...
3. Sync ke mirror: `cp -r . /c/Users/Admin/Documents/GitHub/kasol/control/`
4. Commit dari monorepo root: `cd /c/Users/Admin/Documents/GitHub/kasol && git add control/ && git commit`

**Commit conventions:**
- `admin: <subject>` — general changes
- `control/docs: <subject>` — documentation only
- `control/js: <subject>` — JavaScript modules
- `control/css: <subject>` — styling
...
Control Center Kasir Solo — KASIRSOLO Ecosystem
