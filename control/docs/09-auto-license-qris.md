...
- ⚠️ **P2 unifikasi (dari audit)**: harga juga masih `hardcoded` di
  `control/js/license-ui.js` `PRODUCT_REGISTRY.price`. Sebaiknya harga lisensi
  mengambil `products.price_label`, bukan duplikat — biar satu sumber.
- RLS: `public_read_products` (visible=true) sudah ada → client & shop bisa baca.
...
- `products` & `qris` public (memang perlu ditampilkan klien/shop).
