...
Panduan lengkap integrasi Supabase untuk Control Center dan shop KASIRSOLO.
...
### Anon Key (Public)
- Digunakan oleh **shop** (read-only products)
...
- Digunakan oleh **Control Center** (full CRUD products, clients)
...
- Karena bucket `bukti` privat, admin TIDAK bisa langsung buka via URL biasa → butuh
  **signed URL** (15 menit). Di-generate server-side lewat handler `storageSign`
  (`control/api/rest.js` serverless & `control/server.js` local dev) memakai service_role;
  helper `control/js/api.js` → `supabaseStorageSign()`.
...
| `js/supabase-client.js` | Supabase REST client untuk shop (anon key) |
...
**File utama:** `control/api/license.js` (Vercel Serverless — memegang salt & crypto)

> ⚠️ **Security (Fix C1):** Generate & verifikasi lisensi DIJALANKAN SERVER-SIDE.
> `control/js/license-core.js` adalah pure logic yang dipakai server (`api/license.js`).
...
```javascript
// Konfigurasi salt SERVER-SIDE (control/api/license.js)
const DEFAULT_SALTS = {
...
### Status Phase A
**FIXED:** Sebelumnya Control Center meng-inject **service_role key** ke `window` via `js/env-loader.js` → siapa pun dengan DevTools bisa akses penuh DB. Sekarang service key **tidak pernah** masuk browser; semua operasi Supabase lewat Vercel Serverless Proxy `/api/rest` (service key server-side) + whitelist tabel + gate `ADMIN_API_KEY`.
...
1. **Supabase Auth (Email/Password) + RLS per role**
   ...
   - Anon key tetap dipakai shop + client apps
...
- [x] Update shop fetch dari Supabase
...
| `control/js/license-core.js` | Pure logic HMAC-SHA256 + Base32 (generate/verify) |
| `control/js/clients.js` | CRM module + generate serial di sheet |
