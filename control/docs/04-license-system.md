# Control Center — Sistem Lisensi

Detail lengkap algoritma lisensi HMAC-SHA256 yang terintegrasi di Control Center.
...
> - **Admin `control/` mengelola SEMUA lisensi app klien** (kecuali shop & control):
...
```
  CONTROL CENTER         SUPABASE (Meta/CRM)            KLIEN (Aplikasi)
        │                        │                            │
        │  1. Generate serial    │                            │
        │     (device code)      │                            │
...
> target**. Sampai `control/` tersinkron ke Supabase, langkah ini dilewati dan
...
> bisa memindahkan lisensi antar-unit lewat tombol **"↔️ Unit"** (`control/js/clients.js`
...
*Sistem Lisensi — KASIRSOLO Control Center*
