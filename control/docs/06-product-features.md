# Control Center — Fitur Lengkap

Penjelasan detail setiap modul di Control Center (modular ESM + kaki5 design system).
...
<div id="clientsEmpty" class="empty-state" hidden>
  ...
  <div class="empty-desc">Klien akan otomatis muncul saat ada yang mengisi profil di aplikasi klien (shop / app kaki5).</div>
</div>
```
...
Modul CRUD untuk aplikasi yang tampil di shop.
...
Form untuk mengontrol konten shop + backup/restore.
...
### Shop Config (`.field-grid` 2 kolom Tablet+)
...
## 🔄 Sinkronisasi dengan Shop

| Modifikasi di Control Center | Efek di Shop |
|---------------------|-----------------|
| Tambah/ubah/hapus aplikasi di Katalog | Katalog di shop berubah (refresh halaman) |
| Ubah pengaturan kontak/alamat | Footer & CTA WhatsApp di shop berubah |
| Ubah statistik hero | Counter di hero berubah (refresh halaman) |
| Lead baru dari form trial | Muncul di tab Leads admin |

> **Catatan:** Perubahan katalog & pengaturan memerlukan refresh halaman shop untuk terlihat.

---

*Product Features — KASIRSOLO Control Center*