...
### 6. Agent AI (Draft Otomatis — Human-in-the-Loop)

| Aspek | Detail |
|-------|--------|
| **Peran** | AI menyusun **draft** (pesan WA/laporan/brief SEO) → owner yang memutuskan |
| **Otak** | Omniroute lokal `127.0.0.1:20128` (provider model `auto/*`) |
| **Runner** | `agent/runner.mjs` — claim `agent_tasks` → tanya otak → tulis `agent_outputs` |
| **Seeder** | `agent/enqueue-reaktivasi.mjs` — baca `outreach_contacts` → posang tugas (idempotent) |
| **Meja Review** | Control → halaman Agent (`#agents`): chip filter, statline, preview draft, sheet keputusan |
| **Status** | `draft → approved → dipakai` / `rejected` (bisa balik ke draft) |
| **Keamanan** | RLS tanpa policy publik; runner pakai SERVICE_ROLE lokal; render aman `escapeHtml` |
| **Dokumentasi** | [`control/docs/12-agent-ai.md`](./12-agent-ai.md) — baca sebelum menyentuh |
