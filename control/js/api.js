/**
 * Client helper — control/js/api.js
 * =============================================================================
...
export async function supabaseFetch(path, { method = 'GET', data, headers = {} } = {}) {
  ...
  while (retries < MAX_REFRESH_RETRIES) {
    const r = await fetch('/api/rest', {
      ...
      // headers kustom (mis. Prefer utk upsert) WAJIB ikut di body — proxy
      // membacanya dari body, bukan dari HTTP headers (tanpa ini 409 di baris ada).
