/**
 * Control Center Kasir Solo — Settings Module
 * Settings form for business info, payment, & backup
 * (konten landing pindah ke modul CMS "Konten Web"; link aplikasi
 * dikelola per kartu produk di Katalog)
 */
...
export function initSettings() {
  ...
  // Tab Pengaturan: Umum | Pembayaran | Data
  window.switchSettingsTab = function (tab) {
    document.querySelectorAll('#settingsTabs .content-tab').forEach((b) => {
      const on = b.dataset.stab === tab;
      b.classList.toggle('active', on);
      b.setAttribute('aria-selected', String(on));
    });
    const v = (id) => { const el = document.getElementById(id); if (el) el.hidden = id !== `setView${tab.charAt(0).toUpperCase()}${tab.slice(1)}`; };
    v('setViewUmum'); v('setViewPembayaran'); v('setViewData');
  };
  window.switchSettingsTab('umum');
  document.querySelectorAll('#settingsTabs .content-tab').forEach((b) => {
    b.addEventListener('click', () => window.switchSettingsTab(b.dataset.stab));
  });
...
/** Tarik pengaturan pembayaran dari Supabase (sumber kebenaran cloud) */
