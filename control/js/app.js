...
import { initNavigation, getCurrentScreen } from './navigation.js?v=20260920a';
import { initDashboard } from './dashboard.js?v=20260920a';
import { initCatalog } from './catalog.js?v=20260920a';
import { initContent } from './content.js?v=20260920a';
import { initSettings } from './settings.js?v=20260920a';
import { initClients } from './clients.js?v=20260920a';
import { initOutreach } from './outreach.js?v=20260920a';
import { initAgents } from './agents.js?v=20260920a';
...
window.refreshCurrentScreen = function () {
  ...
  const fn = screen === 'klien' ? window.refreshClients
    : screen === 'outreach' ? window.refreshOutreach
    : screen === 'agents' ? window.refreshAgents
...
/**
 * Sheet & overlay utilities (global)
 */
...
window.closeSheet = function(sheetId) {
  ...
  if (overlay) {
    ...
    setTimeout(() => { overlay.style.display = 'none'; }, 280); // wait for animation
  }
};

window.openMoreMenu = function() {
  window.openSheet('sheetMoreMenu');
};

window.switchScreen = function(screen, clientView) {
  // Import switchScreen from navigation module
  import('./navigation.js').then(({ switchScreen: switchScreenFn }) => {
    switchScreenFn(screen, clientView);
  });
};