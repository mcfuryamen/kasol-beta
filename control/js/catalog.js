...
async function fetchProductsFromSupabase() {
  try {
    const res = await supabaseFetch('/rest/v1/products?select=id,kode_produk,app_type,icon,name,description,price_label,price_before_label,order_index,visible,status,store_url,vercel_url,tx_quota,tutorials&order=order_index.asc');
    if (!res.ok) {
    ...
    return products.map(p => ({
      ...
      orderIndex: p.order_index,
          ...
          tutorials: Array.isArray(p.tutorials) ? p.tutorials : []
        }));
  } catch (error) {
...
/**
 * Video tutorial (products.tutorials jsonb) ↔ teks editor.
 * Format baris: Judul | URL (watch/shorts/youtu.be/embed atau kode <iframe>) | Durasi | Deskripsi
 * Baris tanpa "|" dianggap URL polos. Baris kosong diabaikan.
 * Validasi ketat ada di sisi aplikasi (parseYoutubeId) — editor menyimpan apa adanya.
 */
function tutorialsToLines(list) {
  return (list || []).map(t => {
    if (typeof t === 'string') return t;
    return [t.judul || '', t.url || '', t.durasi || '', t.deskripsi || ''].join(' | ').replace(/(\s\|\s)+$/, '');
  }).join('\n');
}
function linesToTutorials(text) {
  return (text || '').split('\n').map(l => l.trim()).filter(Boolean).map(l => {
    if (!l.includes('|')) return { judul: '', url: l, durasi: '', deskripsi: '' };
    const p = l.split('|').map(s => s.trim());
    return { judul: p[0] || '', url: p[1] || '', durasi: p[2] || '', deskripsi: p[3] || '' };
  }).filter(t => t.url);
}
...
export function renderCatalog() {
  ...
  if (catalogTableBody) {
    catalogTableBody.innerHTML = apps.map((app, idx) => `
      <tr data-idx="${idx}" data-id="${escapeHtml(app.id)}">
        ...
        <td>
          <div class="ct-name">${escapeHtml(app.name || 'Aplikasi Baru')}${app.hot ? ' <span class="hot-badge hot-badge-sm">HOT</span>' : ''}</div>
  ...
  // ── Kartu (mobile) — dense rows ala native, reuse sistem .oc-* outreach ──
  catalogGrid.innerHTML = apps.map((app, idx) => `
    <div class="oc-row" data-idx="${idx}" data-id="${escapeHtml(app.id)}" role="button" tabindex="0" aria-label="Edit ${escapeHtml(app.name || '')}">
      <div class="oc-ava">${appIcon(app.appType, 18)}</div>
      <div class="oc-main">
        <div class="oc-name">${escapeHtml(app.name || 'Aplikasi Baru')}${app.hot ? ' <span class="hot-badge hot-badge-sm">HOT</span>' : ''}</div>
        <div class="oc-sub"><span class="mono">${escapeHtml(app.kodeProduk || app.appType || '—')}</span> · ${formatRupiah(app.price || 0)}${app.txQuota ? ` · ${escapeHtml(String(app.txQuota))}/bln` : ''}</div>
      </div>
      <div class="oc-side">
        ${statusChip(app)}
        <button type="button" class="oc-more" aria-label="Edit ${escapeHtml(app.name || '')}" title="Edit" onclick="event.stopPropagation(); openCatalogPage(${idx})">${icon('pencil', 14)}</button>
      </div>
    </div>
  `).join('');

  // Bind click events for editing
  catalogGrid.querySelectorAll('.oc-row').forEach(card => {
  ...
  page.innerHTML = `
    <div class="cd-head cd-head-sticky">
      ...
      <div class="cd-head-actions">
        ${isEdit ? `<button type="button" class="btn btn-danger btn-sm" onclick="deleteCatalogApp(${idx})">Hapus</button>` : ''}
        <button type="button" class="btn btn-primary btn-sm" onclick="saveCatalogApp(${idx})">${isEdit ? 'Simpan' : 'Tambah'}</button>
      </div>
    </div>

    <div class="cd-sec">
      <div class="cd-sec-t">Identitas</div>
    ...
    <div class="cd-sec">
      <div class="cd-sec-t">Harga &amp; Kuota</div>
      <div class="field-grid">
        <div class="field">
        ...
        <div class="field">
          <label class="field-label">Harga Coret (opsional)</label>
        ...
        <div class="field field-span-2">
          ...
          <small class="field-hint">Tier gratis = N transaksi selesai per bulan kalender, tanpa batas waktu. Kosong → default app (100). Diadjust per pelanggan lewat kartu Klien.</small>
        </div>
        <div class="field">
    ...
    <div class="cd-sec">
      <div class="cd-sec-t">Klasifikasi &amp; Distribusi</div>
      <div class="field-grid">
        <div class="field">
        ...
        <div class="field">
          ...
          <small class="field-hint">Otomatis jadi "Development" jika Live &amp; Vercel kosong.</small>
        </div>
        <div class="field field-span-2">
        ...
        <div class="field field-span-2">
          ...
          <small class="field-hint">Domain preview/staging (opsional).</small>
        </div>
      </div>
    </div>

    <div class="cd-sec">
      <div class="cd-sec-t">🎬 Video Tutorial</div>
      <div class="field-grid">
        <div class="field field-span-2">
          <textarea id="catTutorials" rows="4" placeholder="Cara Buka/Tutup Kas | https://youtube.com/watch?v=xxxxxxxxxxx | 3:12${'' /* opsi: | deskripsi */}">${escapeHtml(tutorialsToLines(app.tutorials))}</textarea>
          <small class="field-hint">Satu video per baris: <b>Judul | URL | Durasi | Deskripsi</b>. Hanya YouTube yang render di aplikasi; lainnya dilewati. Muncul di tab 🎬 Video halaman Bantuan aplikasi ini.</small>
        </div>
      </div>
    </div>
  `;
...
window.saveCatalogApp = async function(idx = null) {
  ...
  const tutorials = linesToTutorials(document.getElementById('catTutorials')?.value || '');
...
/** Hitungan tab + garis ringkasan status per tab aktif */
function renderTabCounts() {
  const set = (id, n) => { const el = document.getElementById(id); if (el) el.textContent = n; };
  const sw = STATE.catalog || [];
  set('catN-sw', sw.length);
  set('catN-hw', hwItems.length);
  set('catN-svc', svcItems.length);

  const line = document.getElementById('catStatusLine');
  if (!line) return;
  const byStatus = (list) => ({
    live: list.filter((x) => resolveProductStatus(x) === 'live').length,
    ready: list.filter((x) => resolveProductStatus(x) === 'ready').length,
    maintenance: list.filter((x) => resolveProductStatus(x) === 'maintenance').length,
    development: list.filter((x) => resolveProductStatus(x) === 'development').length
  });
  const chip = (n, label, cls) => n > 0 ? `<span class="cat-st"><i class="${cls}"></i>${n} ${label}</span>` : '';
  let list, total;
  if (catTab === 'hardware') { list = hwItems; total = hwItems.length; }
  else if (catTab === 'layanan') { list = svcItems; total = svcItems.length; }
  else { list = sw; total = sw.length; }
  const st = byStatus(list);
  line.innerHTML = chip(total, catTab === 'software' ? 'aplikasi' : catTab, '')
    + chip(st.live, 'live', 'is-live')
    + chip(st.ready, 'ready', 'is-ready')
    + chip(st.maintenance, 'maintenance', 'is-maint')
    + chip(st.development, 'development', 'is-dev');
}
...
function renderHw() {
  ...
  grid.innerHTML = hwItems.map((p, i) => `
    <div class="oc-row" data-idx="${i}" role="button" tabindex="0" aria-label="Edit ${escapeHtml(p.name || '')}">
      <div class="oc-ava oc-ava--img">${p.image ? `<img src="${escapeHtml(p.image)}" alt="" loading="lazy">` : '📦'}</div>
      <div class="oc-main">
        <div class="oc-name">${escapeHtml(p.name || '—')}</div>
        <div class="oc-sub">${escapeHtml(hwCatLabel(p.category))} · <span class="mono">${formatRupiah(p.price || 0)}</span></div>
      </div>
      <div class="oc-side">
        ${p.tag ? `<span class="hot-badge">${escapeHtml(p.tag)}</span>` : ''}
        <button type="button" class="oc-more" aria-label="Edit ${escapeHtml(p.name || '')}" title="Edit" onclick="event.stopPropagation(); openHwPage(${i})">${icon('pencil', 14)}</button>
      </div>
    </div>`).join('');
  grid.querySelectorAll('.oc-row').forEach((card) => {
...
window.openHwPage = function (idx = null) {
  ...
  page.innerHTML = `
    <div class="cd-head cd-head-sticky">
      ...
      <div class="cd-head-actions">
        ${isEdit ? `<button type="button" class="btn btn-danger btn-sm" onclick="deleteHw(${idx})">Hapus</button>` : ''}
        <button type="button" class="btn btn-primary btn-sm" onclick="saveHw(${idx})">${isEdit ? 'Simpan' : 'Tambah'}</button>
      </div>
    </div>
    <div class="panel">
...
function renderSvc() {
  ...
  grid.innerHTML = svcItems.map((s, i) => `
    <div class="oc-row" data-idx="${i}" role="button" tabindex="0" aria-label="Edit ${escapeHtml(s.title || '')}">
      <div class="oc-ava">🛠️</div>
      <div class="oc-main">
        <div class="oc-name">${escapeHtml(s.title || '—')}</div>
        <div class="oc-sub">${escapeHtml(s.subtitle || '')}</div>
      </div>
      <div class="oc-side">
        <button type="button" class="oc-more" aria-label="Edit ${escapeHtml(s.title || '')}" title="Edit" onclick="event.stopPropagation(); openSvcPage(${i})">${icon('pencil', 14)}</button>
      </div>
    </div>`).join('');
  grid.querySelectorAll('.oc-row').forEach((card) => {
...
window.openSvcPage = function (idx = null) {
  ...
  page.innerHTML = `
    <div class="cd-head cd-head-sticky">
      ...
      <div class="cd-head-actions">
        ${isEdit ? `<button type="button" class="btn btn-danger btn-sm" onclick="deleteSvc(${idx})">Hapus</button>` : ''}
        <button type="button" class="btn btn-primary btn-sm" onclick="saveSvc(${idx})">${isEdit ? 'Simpan' : 'Tambah'}</button>
      </div>
    </div>
    <div class="panel">
      <div class="panel-body">
