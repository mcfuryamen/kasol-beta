...
import { setSetting, getSetting, toast, openOverlay, closeSheet, escapeHtml } from './utils.js';
import { refreshTxQuotaConfig, syncLicenseStatusThrottled, fetchProductSalt } from './license.sync.js';
// Jatah OFFLINE (port kaki5 2026-09-20): tambal anti-kebocoran tier gratis utk
// device yang tak pernah online. Modul PURE — hmac/storage diinjecte.
import {
  quotaEnsure, quotaTickTx, quotaBudgetStatus,
} from './quota.offline.js';
...
export async function getDeviceIdentity() {
  ...
  let stored = await getSetting('deviceIdentity', null) || {};
  // Jaring cadangan (2026-09-16): IndexedDB (Dexie) sering di-evict otomatis oleh
  // OS Android untuk PWA saat tekanan penyimpanan, sementara localStorage biasanya
  // bertahan. Kalau identitas beku hilang dari Dexie, pulihkan dari mirror
  // localStorage supaya deviceCode tidak lahir ulang tiap sesi (gejala "ID
  // Perangkat selalu berubah"). Aditif — tidak mengubah kode yang sudah beku.
  if (!stored.deviceCode) {
    try {
      const mirror = JSON.parse(localStorage.getItem('ksr:deviceIdentity') || 'null');
      if (mirror && mirror.deviceCode) stored = mirror;
    } catch (_) { /* localStorage gagal — abaikan */ }
  }
  ...
  // Mirror ke localStorage (cadangan anti-evict IndexedDB). Pasang-fingerprint
  // disimpan bersama kode agar cek "terjangkar" tetap konsisten saat dipulihkan.
  try {
    localStorage.setItem('ksr:deviceIdentity', JSON.stringify({
      installId: identity.installId, deviceCode: identity.deviceCode,
      fingerprint: identity.fingerprint, candidateCode: identity.candidateCode,
      fpVersion: identity.fpVersion, legacyDeviceCode: identity.legacyDeviceCode
    }));
  } catch (_) { /* quota / mode privat — abaikan, Dexie tetap sumber utama */ }
  return identity;
}

// ── Re-derive deviceCode era fingerprint V4 (adopsi kaki5
//    getLegacyV3/V4DeviceCode, 2026-09-13) ────────────────────────────────────
// Era V4 (2026-09-04) memakai resolusi layar MENTAH dan masih punya DUA jalur
// digest: SHA-256 di secure context, FNV-1a di http://LAN. Serial terbitan era
// itu bisa terikat ke salah satu rasa, dan layar bisa berada di orientasi
// berbeda saat aktivasi vs saat validasi → kandidat mencakup SEMUA:
// 2 orientasi × 2 digest. Semuanya deterministik, jadi masa tenggang serial
// era V4 tetap jalan walau legacyDeviceCode hilang (data dibersihkan /
// browser baru) — persis filosofi kaki5. Era deviceId-acak (pra-fingerprint)
// memang tak bisa di-re-derive — itu tetap lewat legacyDeviceCode tersimpan
// (lihat validateLicenseKeyV2).
function fnv1aHistorical(bytes) {
...
export async function getLegacyV4DeviceCodes() {
  const nav = (typeof navigator !== 'undefined') ? navigator : {};
  const base = [
  ...
  let screenPart = 'sc:na';
  try { screenPart = String(screen.width) + 'x' + String(screen.height); } catch (_) { /* tanpa layar */ }
  const variants = [screenPart];
  const m = screenPart.match(/^(\d+)x(\d+)$/);
  if (m && m[1] !== m[2]) variants.push(m[2] + 'x' + m[1]); // orientasi diterbalikkan
  const codes = new Set();
  for (const sp of variants) {
    const enc = new TextEncoder().encode('KSR-FP-V4|' + base.concat(sp).join('|'));
    try {
      if (typeof crypto !== 'undefined' && crypto.subtle) {
        codes.add(getDeviceCode(b32Encode(new Uint8Array(await crypto.subtle.digest('SHA-256', enc)), 12)));
      } else {
        codes.add(getDeviceCode(b32Encode(sha256PureBytes(enc), 12)));
      }
    } catch (_) { /* rasa SHA gagal — lewati */ }
    try { codes.add(getDeviceCode(b32Encode(fnv1aHistorical(enc), 12))); } catch (_) { /* diabaikan */ }
  }
...
export async function ensureUnitId() {
  ...
  // IMMUTABLE ala kaki5 (getUnitId): lahir SATU KALI lalu dipertahankan —
  // begitu terikat serial/profil di cloud, TIDAK boleh lahir ulang walau data
  // lokal sebagian hilang. unitId tersimpan hilang tapi lisensi masih mengikat
  // serial → PULIHKAN dari identitas perangkat (defense-in-depth, jangan
  // dianggap perangkat baru); reassign unit_id hanya lewat device_assign.
  // Sumber kode = identitas HIDUP (getDeviceIdentity), bukan cache boot.
  const { deviceCode } = await getDeviceIdentity();
  const fresh = 'KSR-' + deviceCode;
...
export async function incrementTxCount() {
  ...
  // Jatah offline: counter kumulatif seumur instal ikut naik (tak pernah garis)
  try {
    const { deviceCode } = await getDeviceIdentity();
...
export async function validateLicenseKeyV2(rawKey, myDeviceCode, activationDate) {
  ...
  if (d1 + '-' + d2 !== myDeviceCode) {
    // Masa tenggang migrasi identitas (deviceId-acak → fingerprint V4 → V5),
    // pola kaki5 getLegacyV3/V4DeviceCode: serial yang TERBIT sebelum migrasi
    // terikat kode era lama dan tetap diterima supaya perangkat lama tidak
    // terkunci. Dua jalur kandidat:
    //   1. legacyDeviceCode tersimpan — kode era deviceId-acak browser ini
    //      (tidak deterministik, tak bisa dihitung ulang; hilang bila data
    //      dibersihkan);
    //   2. re-derive DETERMINISTIK kode era fingerprint V4 dari hardware
    //      (getLegacyV4DeviceCodes) — masa tenggang tetap jalan meski storage
    //      kosong / browser baru, seperti kaki5.
    const legacy = ((await getSetting('deviceIdentity', null)) || {}).legacyDeviceCode || '';
    if (!legacy || d1 + '-' + d2 !== legacy) {
      const legacyV4 = await getLegacyV4DeviceCodes();
      if (!legacyV4.includes(d1 + '-' + d2)) return { valid: false, reason: 'device' };
    }
  }
...
export async function getLicenseStatus() {
  ...
  // ID Perangkat yang ditampilkan = kode perangkat AKTIF (dari identitas boot),
  // BUKAN lic.deviceCode — nilai itu tercemat saat aktivasi/adopsi cloud dan
  // bisa basi setelah konvergensi identitas legacy (port kaki5
  // getLicenseStatus: kartu/pengaturan menampilkan kode lama padahal
  // identitas sudah konvergen).
  const deviceCode = getDeviceIdForLicense();
  ...
  if (lic.status === 'trial') {
    ...
    // Jatah OFFLINE (port kaki5): offline >= 30 hari JAU (lifetime >= 300 trx
    // ATAU usia >= 3 bulan) → dikunci; check-in online SATU kali unblocks.
    try {
      const { meta, tampered } = await quotaEnsure(getSetting, setSetting, hmacSignature, deviceCode, nowMs);
      if (tampered) return { status: 'expired', deviceCode, trialExpired: true, offlineBudget: 'tampered', txRemaining: Math.max(0, remaining), txQuota: quota, txUsed: used };
      const bud = quotaBudgetStatus(meta, nowMs);
      if (bud.block) return { status: 'expired', deviceCode, trialExpired: true, offlineBudget: bud.reason, txRemaining: Math.max(0, remaining), txQuota: quota, txUsed: used };
    } catch (_) { /* storage gagal → lanjut (gate kuota bulan masih jaga) */ }
...
// Stepper 4 tahap ala kaki5 (Gratis→Beli→Proses→Aktif) — paritas visual kartu lisensi.
...
// Aktivasi manual (input kode) & tombol WhatsApp DIHAPUS (keputusan pemilik
// 2026-09-14): semua proses lisensi terpusat di aplikasi Control — beli lewat
// sheet ini (QRIS/rekening + bukti → verifikasi admin) atau diaktifkan admin.
function licenseStatusHtml(st){
  if(st.status === 'active'){
    const expTxt = st.expiryLabel ? 'Masa berlaku: ' + escapeHtml(st.expiryLabel) : 'Berlaku seumur hidup';
    return `<div class="card license-card-active">
      ${licenseSteps(4)}
      <div class="license-icon">✅</div>
      <div class="badge green compact">✓ Lisensi Aktif</div>
      <p class="license-key"><b>${escapeHtml(st.serial || '-')}</b></p>
      ...
      <div class="license-active-actions"><button class="btn btn-outline" onclick="window._ksr_refreshLicenseStatus()">🔄 Refresh Status</button></div>
    </div>`;
  }
...
export function openLicenseSheet(){
  getLicenseStatus().then(st => {
    ...
    if(body) body.innerHTML = licenseStatusHtml(st);
    openOverlay('sheetLicense');
  });
}

// Refresh Status (kartu lisensi aktif, paritas kaki5) — tarik cloud + render ulang.
export async function refreshLicenseStatus(){
  toast('Memeriksa status lisensi...');
  try { await checkLicenseGate(); } catch(_) {}
  toast('Status lisensi terbaru dimuat');
}

// Aktivasi manual dihapus (2026-09-14, keputusan pemilik): lisensi hanya
// terpusat — beli via sheet (bukti → verifikasi admin) atau di-set admin
// di aplikasi Control. validateLicenseKeyV2/V1 tetap tersimpan untuk
// memverifikasi serial era lama yang diadopsi dari cloud (persistCloudLicense).

// Global exports for onclick
window._ksr_refreshLicenseStatus = refreshLicenseStatus;
