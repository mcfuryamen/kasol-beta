...
//
// FIX v67: repo emsifa direstrukturisasi upstream (2026-09-05, PR #43) — path lama
...
const cache = {};
const CACHE_KEY = 'ksr-region-cache-v1';
...
function esc(s) {
  // Entity map sungguhan — dulu nilainya menyalin karakter itu sendiri
  // (no-op) sehingga nama wilayah dengan & < > " merusak markup option.
...
export function setupRegionPicker({ provSel, kabSel, kecSel, desaSel, state }) {
  ...
  // Guard idempoten: jangan pasang listener berulang tiap sheet alamat dibuka
  // (elemen select sama dipakai ulang → kebocoran listener). Tandai sekali saja.
