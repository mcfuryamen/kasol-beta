...
import { getSetting, setSetting, toast, openOverlay, closeSheet, showLoading, hideLoading, escapeHtml } from './utils.js';
...
export async function openPurchaseSheet(){
  ...
  openOverlay('sheetPurchase'); // TERBUKA SEKETIKA — dulu overlay baru muncul
  // setelah fetch pembayaran selesai (±1-2 dtk) → klik terasa tidak merespon
  // (laporan pemilik 2026-09-14; paritas kaki5 yang membuka sheet dulu).
  ...
  // M6 (audit 2026-09-05 kaki5, port rosok 2026-09-14): qrisUrl admin bisa
  // arbitrary — wajib prefix https://; nilai bank/rekening/nama di-escape.
  const safeQrisUrl = (typeof pay.qrisUrl === 'string' && /^https:\/\//.test(pay.qrisUrl)) ? pay.qrisUrl : '';
  const qrisHtml = safeQrisUrl
    ? `<img src="${safeQrisUrl}" style="width:100%;max-width:280px;border-radius:12px;margin-bottom:10px" alt="QRIS">
       <div style="text-align:center;margin-bottom:12px"><a href="${safeQrisUrl}" download="qris-kasirrosok.png" class="btn btn-outline">⤓ Unduh QRIS</a></div>`
  ...
  const bankHtml = (pay.bank || pay.accountNumber || pay.accountName)
    ? `<div style="background:var(--paper);border:1px solid var(--line);border-radius:12px;padding:12px;margin-bottom:12px">
         <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:6px"><span style="color:var(--ink-soft)">Bank</span><b>${escapeHtml(pay.bank) || '—'}</b></div>
         <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:6px"><span style="color:var(--ink-soft)">No. Rekening</span><span style="font-weight:800;font-family:monospace">${escapeHtml(pay.accountNumber) || '—'}</span></div>
         <div style="display:flex;justify-content:space-between;font-size:13px"><span style="color:var(--ink-soft)">Atas Nama</span><b>${escapeHtml(pay.accountName) || '—'}</b></div>
       </div>`
    : '';
    ...
    ? `<div style="background:var(--paper);border:1px solid var(--line);border-radius:12px;padding:12px 14px;margin-bottom:12px">
         ...
         <div style="display:flex;justify-content:space-between;align-items:center">
           ...
           <span style="display:inline-flex;align-items:baseline;gap:8px">
             ${pay.priceBeforeLabel ? `<s style="font-size:13px;color:var(--brand);font-weight:600">${escapeHtml(pay.priceBeforeLabel)}</s>` : ''}
             <span style="font-size:18px;font-weight:800;color:var(--green)">${escapeHtml(pay.priceLabel)}</span>
           </span>
         </div>
       </div>`
    : '';
  ...
  body.innerHTML = `
    ...
    <div style="padding:10px 12px;background:#FFF3E3;border-radius:10px;font-size:12.5px;color:var(--ink)">
      ...
      2. Transfer sesuai nominal (${escapeHtml(pay.priceLabel) || 'lihat info harga'})<br>
...
export async function submitPurchase(){
  ...
  try {
    // Anti-enumerasi (audit keamanan 2026-09-14): bucket bukti public-read —
    // suffix acak membuat URL tidak dapat ditebak meski unitId & jam upload diketahui.
    const rnd = Array.from(crypto.getRandomValues(new Uint8Array(6)), b => b.toString(36)).join('').slice(0,8);
    const fileName = `${unit_id}_${Date.now()}_${rnd}.jpg`;
