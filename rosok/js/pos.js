...
import { setPrintNotaData } from './printer.js';
...
export function renderCartChips(){
  ...
  row.innerHTML = cart.length ? cart.map((c,i) => `
    <div class="cart-chip">${escapeHtml(c.emoji||'♻️')} ${escapeHtml(c.nama)} · ${fmtKg(c.berat)}<span class="x" onclick="removeCartItem(${i})">✕</span></div>
...
export function renderCartStep2(){
  ...
  list.innerHTML = cart.map((c,i) => `
    <div class="cart-compact-item">
      <div class="item-emoji">${escapeHtml(c.emoji||'♻️')}</div>
      <div class="item-text">
        <div class="item-name">${escapeHtml(c.nama)}</div>
...
export async function saveTransaksi(){
  ...
  if(licSt.status === 'expired'){
    if (licSt.offlineBudget) {
      ...
      toast('Jatah offline habis — hubungkan internet SEKALI untuk verifikasi & lanjuk bertransaksi 📶');
      openLicenseSheetRef();
      return;
    }
  ...
  try {
    ...
    await db.transaction('rw', db.transaksi, db.transaksiItem, db.kategori, db.kas, async () => {
      ...
      if(dibayarkan > 0){
        // Tandai metode: transfer → TIDAK geser laci (masuk "Dompet digital"); tunai/DP tempo → laci.
        await db.kas.add({ tanggal: now, tipe: activeTransTipe==='beli'?'keluar':'masuk', jumlah: dibayarkan, metodeBayar: bayarMetode === 'transfer' ? 'transfer' : 'tunai', keterangan: (activeTransTipe==='beli' ? 'Pembelian rosok' : 'Penjualan rosok') + (bayarMetode==='tempo' ? ' (DP tempo)' : '') + (bayarMetode==='transfer' ? ' (Transfer)' : '') + (namaKontak ? ' - '+namaKontak : ''), refTransaksiId: transaksiId });
      }
    });
...
export function renderNota(data){
  ...
  if(!el) return;
  // Nota aktif utk tombol cetak dual-path (BLE thermal / browser).
  setPrintNotaData(data);
  const biz = (SETTINGS && SETTINGS.bizName) || 'Kasir Solo - Rosok';
  const itemsHtml = data.items.map(it => `
    <div class="nota-line">
      <span>${escapeHtml(it.emoji||'')} ${escapeHtml(it.nama)} (${fmtKg(it.berat)})</span><span>${fmtRupiah(it.subtotal)}</span>
    </div>`).join('');
