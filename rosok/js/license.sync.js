...
import { quotaMarkOnline } from './quota.offline.js';
...
// Control Center. Tidak pernah melempar.
...
export async function persistCloudLicense(cloud) {
  ...
  let deviceCode = local.deviceCode || '';
  if (!deviceCode) {
    // Identitas HIDUP, bukan cache boot (paritas kaki5 persistCloudLicense) —
    // adopsi tanpa lisensi lokal sebelumnya tetap mencatat deviceCode sah.
    try {
      const { getDeviceIdentity } = await import('./license.js');
      deviceCode = (await getDeviceIdentity()).deviceCode;
    } catch (_) { /* storage gagal — biarkan kosong */ }
  }
  const lic = {
    status: 'active',
...
export async function reanchorUnitId({ force = false } = {}) {
  ...
  // Pagar anti-osilasi (2026-09-06, kasus satu perangkat dua deviceCode):
  // begitu perangkat pernah konvergen ke kanonik X, jangan konvergen lagi ke
  // kanonik Y yang berbeda — dua kanonik bergantian berarti sinyal fingerprint
  // geser bolak-balik dan migrasi hanya memindah-mindahkan baris. Konvergensi
  // ke target baru hanya lewat force (Pengaturan → Cek Data Online).
  // PENGECUALIAN (2026-09-13, audit multi-identitas): identitas yang sudah
  // TERFROZEN era V5 (berpenanda fpVersion:'V5' DAN terjangkar) tidak bisa
  // bergeser lagi — kanoniknya stabil, jadi guard justru menghalangi
  // perbaikan sekali-jalan (kasus nyata: origin utama pemilik macet di unit
  // era V4 KSR-00KN-MBNA selagi device_code-nya sudah 00WB-A538, karena
  // anchor lama memblokir konvergensi selamanya → satu PC beberapa baris).
  const lastAnchor = await getSetting('unitReanchor', null);
  if (lastAnchor && lastAnchor.to && lastAnchor.to !== canonical && !force) {
    let frozenStable = false;
    try {
      const { getDeviceCode } = await import('./license.js');
      frozenStable = identity.fpVersion === 'V5' && !!identity.fingerprint &&
        getDeviceCode(identity.fingerprint) === identity.deviceCode;
    } catch (_) { /* modul gagal — guard tetap berlaku */ }
    if (!frozenStable) return { ok: false, reason: 'oscillation-guard' };
  }
  let s = null;
  ...
  if (String(migErr.code || '') === '23505' || /duplicate key/i.test(String(migErr.message || ''))) {
    ...
    if (adopt) {
      ...
      // Baris kanonik KOSONG profilnya (perangkat baru / lahir dari pemulihan
      // hapus-baris) → seed SEKALI dengan profil lokal via pushProfile: kalau
      // tidak, pull cloud-truth berikutnya menimpa profil lokal dengan KOSONG
      // (aturan C2v2) dan Profil usaha "hilang" di browser pengadopsi.
      if (!String(row.nama_usaha || '').trim() && !String(row.no_whatsapp || '').trim()) {
        try { await pushProfile(); } catch (_) { /* seed gagal — bisa diulang dari Cek Data Online */ }
      }
...
async function readClientRow(sb, unitId) {
  const { data, error } = await sb
    .from('clients')
    .select('license_status, license_serial, license_expires_at, first_seen, tx_month, tx_used, tx_adjust, tx_updated_at, nama_usaha, nama_pemilik, no_whatsapp, provinsi_id, provinsi, kabkota_id, kabkota, kecamatan_id, kecamatan, desa_id, desa, alamat_detail, install_id, last_seen, browser, os, device_type, user_agent')
    .eq('unit_id', unitId)
    .eq('app_type', APP_TYPE)
    .maybeSingle();
...
export async function syncLicenseStatus(unitId, licenseApi) {
  ...
  if (result.kind === 'not-found') {
    ...
    try {
      ...
      // Identitas & telemetri ikut sejak baris lahir (paritas buildPayload
      // kaki5): dulu self-insert TANPA install_id/last_seen/telemetri, dan
      // satu-satunya jalur yang menulisnya (pushProfile) bisa lolos duluan
      // via guard "no-profile" → baris rosok lahir tanpa install_id selamanya.
      const insPayload = {
        unit_id: unitId, app_type: APP_TYPE, device_code: licenseApi.getDeviceCode(),
        user_id: _uid, tx_used: 0, tx_adjust: 0,
        install_id: await getSetting('deviceId', ''), last_seen: new Date().toISOString()
      };
      try {
        const dev = getDeviceInfo();
        insPayload.browser = dev.browser; insPayload.os = dev.os;
        insPayload.device_type = dev.deviceType; insPayload.user_agent = dev.userAgent;
      } catch (_) { /* deteksi gagal → biarkan kosong */ }
      const ins = await sessionSb.from('clients').insert(insPayload);
  ...
  // Jatah OFFLINE (port kaki5): roundtrip sukses = check-in online — 
  // lastOnlineAt segar → budget offline otomatis ter-barsen (persis alur).
  try {
    const { hmacSignature, getDeviceIdentity } = await import('./license.js');
    const { deviceCode } = await getDeviceIdentity();
    await quotaMarkOnline(getSetting, setSetting, hmacSignature, deviceCode, Date.now());
  } catch (_) { /* non-kritikel */ }

  // (A0) Backfill identitas & telemetri baris lama (2026-09-13): baris yang
  // lahir sebelum fix self-insert tidak punya install_id (kasus nyata: baris
  // era V4 pemilik) — lengkapi SEKALI saat sync penuh berhasil. Klaim di atas
  // sudah memindahkan user_id ke sesi ini, jadi update lolos RLS.
  try {
    if (!cloud.install_id) {
      const touch = { install_id: await getSetting('deviceId', ''), last_seen: new Date().toISOString() };
      try {
        const dev = getDeviceInfo();
        touch.browser = dev.browser; touch.os = dev.os;
        touch.device_type = dev.deviceType; touch.user_agent = dev.userAgent;
      } catch (_) { /* opsional */ }
      const { error: touchErr } = await sessionSb.from('clients').update(touch)
        .eq('unit_id', unitId).eq('app_type', APP_TYPE);
      if (!touchErr) cloud.install_id = touch.install_id;
      else console.warn('backfill install_id:', touchErr.message || touchErr);
    }
  } catch (_) { /* non-fatal */ }
