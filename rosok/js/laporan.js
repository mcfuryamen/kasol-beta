...
import { fmtRupiah, fmtKg, fmtDate, escapeHtml, openOverlay, closeSheet, toast } from './utils.js';
import { fiturKasAktif, openKasForm } from './kas.js';
...
document.addEventListener('click', (e) => {
  ...
  // Target yang sudah TER-DETACH = klik internal yang barusan me-render ulang
  // DOM-nya sendiri (toggle label / pick tanggal / nav kalender) — BUKAN klik
  // di luar kartu. Dulu target detached dianggap di luar filter → picker yang
  // baru saja dibuka oleh klik itu langsung ditutup lagi oleh event yang sama
  // (event berlanjut menggelembung ke document SETELAH innerHTML diganti)
  // → tombol tanggal terlihat mati total di semua periode.
  const t = e.target;
  if(!(t && t.isConnected)) return;
  const filter = document.getElementById('screenLaporanFilter');
  if(filter && !filter.contains(t)){ _pickerOpen = false; renderDateNav(); }
});
...
// Adopsi flow kaki5 (setor konsinyasi → form pengeluaran prefill): tombol
// Lunasi TIDAK membuka sheet khusus lagi — ia membuka form Catat Kas utama
// dengan tab benar (utang = Pengeluaran, piutang = Pemasukan), kategori
// 'Pelunasan Utang/Piutang', keterangan otomatis, dan jumlah terisi sisa.
// kas.js menyimpannya sebagai pelunasan atomik (kas + sisa transaksi).
export function openLunasi(id){
  db.transaksi.get(id).then(t => {
    if(!t || (t.sisa||0) <= 0){ toast('Transaksi ini sudah lunas'); return; }
    openKasForm({
      tipe: t.tipe==='beli' ? 'keluar' : 'masuk',
      kategori: t.tipe==='beli' ? 'Pelunasan Utang' : 'Pelunasan Piutang',
      keterangan: (t.tipe==='beli' ? 'Pelunasan utang' : 'Pelunasan piutang') + (t.kontakNama ? ' - '+t.kontakNama : '') + ' · ' + fmtDate(t.tanggal),
      jumlah: t.sisa,
      lunasiId: t.id
    });
  });
...
export async function renderLaporan(){
  ...
  if(kasList) kasList.innerHTML = allKas.sort((a,b)=>new Date(b.tanggal)-new Date(a.tanggal)).slice(0,8).map(k=>`
    <div class="row-item"${k.manual ? ` style="cursor:pointer" onclick="window._ksr_editKasManual(${k.id})"` : ''}>
      <div class="row-icon ${k.tipe==='masuk'?'kas-masuk':'kas-keluar'}">${k.tipe==='masuk'?'⬆️':'⬇️'}</div>
      <div class="row-body"><div class="row-title">${escapeHtml(k.keterangan||'-')}${k.manual ? ' <span class="badge orange">manual</span>' : ''}</div><div class="row-sub">${fmtDate(k.tanggal)}${k.kategori ? ' · ' + escapeHtml(k.kategori) : ''}${k.metodeBayar==='transfer' ? ' · transfer' : ''}</div></div>
  ...
  const kasHint = document.getElementById('kasListHint');
  if(kasHint) kasHint.textContent = allKas.some(k=>k.manual) ? 'Ketuk catatan bertanda manual untuk mengubah/menghapus.' : '';

  const shiftList = document.getElementById('kasShiftHistoryList');
  // Baris bisa diklik → sheet Detail Riwayat Kas (port kaki5: baris riwayat
  // shift dulu mati, sekarang membuka rincian angka dihitung ulang).
  if(shiftList) shiftList.innerHTML = (await db.kasShift.orderBy('waktuBuka').reverse().toArray()).slice(0,10).map(s=>{
    ...
    const selisihTxt = buka || s.selisih === null || s.selisih === undefined ? '' :
      ` · Selisih <b style="color:${s.selisih === 0 ? 'var(--green)' : 'var(--red)'}">${s.selisih > 0 ? '+' : s.selisih < 0 ? '−' : ''}${fmtRupiah(Math.abs(s.selisih))}</b>`;
    return `
    <div class="row-item" style="cursor:pointer" onclick="window._ksr_showKasShiftDetail(${s.id})">
      <div class="row-icon ${buka ? 'kas-masuk' : 'kas-keluar'}">${buka ? '🔓' : '🔒'}</div>
      <div class="row-body">
        <div class="row-title">${fmtDate(s.waktuBuka)}</div>
        <div class="row-sub">Modal ${fmtRupiah(s.modalAwal || 0)}${buka ? '' : ` · Sistem ${fmtRupiah(s.kasSistemAkhir || 0)} · Fisik ${fmtRupiah(s.kasFisikAkhir || 0)}`}${selisihTxt}</div>
        ${s.catatanTutup ? `<div class="row-sub" style="font-style:italic">"${escapeHtml(s.catatanTutup)}"</div>` : ''}
      </div>
      <div class="kas-shift-go"><span class="badge ${buka ? 'green' : ''}">${buka ? 'Berjalan' : 'Tutup'}</span><span class="kas-shift-chev">Detail ›</span></div>
    </div>`;
