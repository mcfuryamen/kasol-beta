...
import { openShiftCache, kasFormTipe, setOpenShiftCache, setKasFormTipe, SETTINGS } from './app-state.js';
import { fmtRupiah, fmtDate, unformatRupiah, toast, openOverlay, closeSheet, getSetting, todayStr, escapeHtml } from './utils.js';
import { showConfirm } from './confirm.js';

// Konteks "Detail Riwayat Kas" aktif (mutasi + ringkasan) → dipakai tombol
// "Detail Mutasi" (render inline) & "Cetak" (printKasShift) di sheet yang sama.
let _shiftDetailCtx = null;
...
// Hanya catatan TUNAI yang menggeser uang laci (adopsi kaki5 v164): pembayaran
// transfer masuk "Dompet digital" (rekening), bukan laci — supaya rekonsiliasi
// fisik akurat. Baris transfer BARU ber-label metodeBayar='transfer'; baris
// transfer LAMA (pra-label) dikenali dari penanda '(Transfer)' di keterangan.
function menggeserLaci(k){
  if (k.metodeBayar && k.metodeBayar !== 'tunai') return false; // transfer eksplisit (transaksi baru & catatan manual)
  if (/\(Transfer\)/.test(k.keterangan || '')) return false;      // warisan: baris transaksi transfer lama tanpa label metode
  return true;
}
...
export async function hitungRingkasanShift(waktuMulai, sampai){
  ...
  const semua = (await db.kas.where('tanggal').aboveOrEqual(batasBawah).toArray())
    .filter(k => new Date(k.tanggal) <= sampai);
  let masuk = 0, keluar = 0, digitalMasuk = 0, digitalKeluar = 0;
  for(const k of semua){
    if(menggeserLaci(k)){ if(k.tipe === 'masuk') masuk += (k.jumlah || 0); else keluar += (k.jumlah || 0); }
    else { if(k.tipe === 'masuk') digitalMasuk += (k.jumlah || 0); else digitalKeluar += (k.jumlah || 0); }
  }
  ...
  const jumlahTransaksi = (await db.transaksi.toArray())
    ...
    .length;
  return { masuk, keluar, saldoSistem: masuk - keluar, digitalMasuk, digitalKeluar, digitalNet: digitalMasuk - digitalKeluar, jumlahTransaksi };
}
...
export async function showKasShiftDetail(id){
  ...
  // Dompet digital (transfer) — uang yang tak pernah lewat laci. Sudah dihitung
  // hitungRingkasanShift sebagai baris kas non-laci (konsisten & saling-lepas
  // dengan angka laci; mencakup data lama via penanda '(Transfer)').
  const digitalMasuk = r.digitalMasuk, digitalKeluar = r.digitalKeluar, digitalNet = r.digitalNet;
  if (digitalMasuk || digitalKeluar) {
    html += shiftDetailRow('📱 Dompet digital — masuk (transfer)', '+' + fmtRupiah(digitalMasuk), 'var(--green)');
    html += shiftDetailRow('📱 Dompet digital — keluar (transfer)', '−' + fmtRupiah(digitalKeluar), 'var(--red)');
    html += shiftDetailRow('Saldo digital di rekening', fmtRupiah(digitalNet));
  }
  ...
  // Daftar mutasi kas selama shift (dipakai "Detail Mutasi" inline & cetak).
  const batasBawah = new Date(shift.waktuBuka).toISOString();
  const mutasi = (await db.kas.where('tanggal').aboveOrEqual(batasBawah).toArray())
    .filter(k => menggeserLaci(k) && new Date(k.tanggal) <= new Date(tutupMs))
    .sort((a, b) => new Date(b.tanggal) - new Date(a.tanggal));

  // Konteks shift aktif → toggle mutasi + cetak.
  _shiftDetailCtx = {
    mutasi,
    bizName: (SETTINGS && SETTINGS.bizName) || 'Kasir Solo - Rosok',
    alamat: [SETTINGS.alamatDetail, SETTINGS.bizDesa, SETTINGS.bizKecamatan, SETTINGS.bizKabkota, SETTINGS.bizProvinsi].filter(Boolean).join(', '),
    tgl: fmtDate(shift.waktuBuka),
    status: masihBuka ? 'Shift masih berjalan' : 'Shift ditutup',
    summary: [
      { label: 'Modal awal', value: fmtRupiah(shift.modalAwal || 0) },
      { label: 'Kas masuk (' + r.jumlahTransaksi + ' transaksi)', value: '+' + fmtRupiah(r.masuk) },
      { label: 'Kas keluar', value: '−' + fmtRupiah(r.keluar) },
      { label: masihBuka ? 'Kas sistem saat ini' : 'Kas sistem akhir', value: fmtRupiah(sistemResmi) },
      ...(masihBuka ? [] : [
        { label: 'Kas fisik dihitung', value: fisikResmi == null ? '— tidak diisi —' : fmtRupiah(fisikResmi) },
        { label: 'Selisih', value: (selisih > 0 ? '+' : '') + fmtRupiah(selisih) },
      ]),
      ...( (digitalMasuk || digitalKeluar) ? [
        { label: 'Dompet digital masuk (transfer)', value: '+' + fmtRupiah(digitalMasuk) },
        { label: 'Dompet digital keluar (transfer)', value: '−' + fmtRupiah(digitalKeluar) },
        { label: 'Saldo digital (rekening)', value: fmtRupiah(digitalNet) },
      ] : [] ),
    ],
  };

  // Akordeon "Detail Mutasi" di dalam body (bukan tombol aksi).
  html += `<div class="kas-mutasi-acc" id="kasMutasiAcc">` +
    `<div class="kas-mutasi-acc-head" onclick="window._ksr_toggleKasMutasi()"><span>🧾 Detail Mutasi</span><span class="kas-mutasi-chev">▾</span></div>` +
    `<div id="kasShiftMutasi" class="compact-list" style="display:none"></div>` +
  `</div>`;

  // Action bar 50:50 — Cetak (kiri) + Tutup Kas (kanan, hanya saat shift masih buka).
  html += `<div class="kas-shift-actions">` +
    `<button class="btn btn-secondary" onclick="window._ksr_cetakKasShift()">🖨️ Cetak</button>` +
    (masihBuka ? `<button class="btn btn-primary" onclick="window._ksr_closeKasShiftDetail();window._ksr_openTutupKasSheet()">🔒 Tutup Kas</button>` : '') +
  `</div>`;
...
// Buka/tutup akordeon "Detail Mutasi" di body sheet (render sekali).
export function toggleKasMutasi(){
  const box = document.getElementById('kasShiftMutasi');
  const acc = document.getElementById('kasMutasiAcc');
  if(!box) return;
  const open = box.style.display === 'block';
  if(open){ box.style.display = 'none'; if(acc) acc.classList.remove('open'); return; }
  if(box.dataset.rendered !== '1'){
    const m = (_shiftDetailCtx && _shiftDetailCtx.mutasi) || [];
    box.innerHTML = m.length ? m.map(k => `
      <div class="row-item">
      ...
      </div>`).join('') : '<div class="hint" style="padding:8px 0">Tidak ada mutasi pada shift ini.</div>';
...
// Cetak laporan shift (ringkasan + mutasi) — jalur ganda ala nota (BLE/browser).
export async function cetakKasShift(){
  if(!_shiftDetailCtx){ toast('Buka detail shift dulu'); return; }
  const c = _shiftDetailCtx;
  const mutasi = (c.mutasi || []).map(k => ({ tipe: k.tipe, jumlah: k.jumlah, keterangan: k.keterangan || '-', waktu: fmtDate(k.tanggal) }));
  try {
    const { printKasShift } = await import('./printer.js');
    await printKasShift({ bizName: c.bizName, alamat: c.alamat, tgl: c.tgl, status: c.status, summary: c.summary, mutasi });
  } catch(e){ console.warn('[KAS] cetak shift gagal:', e?.message || e); toast('Gagal mencetak'); }
}
...
export async function tutupKas(){
  ...
  db.kasShift.update(shift.id, {
    ...
    // Tawarkan cetak laporan shift yang baru ditutup (pemilik 2026-09-16).
    const mauCetak = await showConfirm({ icon: '🖨️', text: 'Cetak laporan kas yang baru ditutup?', okLabel: 'Cetak', cancelLabel: 'Tidak' });
    if(mauCetak) await cetakShiftTertutup({ shift, sistem, fisik, selisih });
  }).catch(e => console.error('Tutup Kas error:', e));
}

// Bangun konteks laporan shift yang baru ditutup lalu cetak (ringkasan + mutasi).
async function cetakShiftTertutup({ shift, sistem, fisik, selisih }){
  const r = await hitungRingkasanShift(shift.waktuBuka, new Date());
  const batasBawah = new Date(shift.waktuBuka).toISOString();
  const mutasi = (await db.kas.where('tanggal').aboveOrEqual(batasBawah).toArray())
    .filter(k => menggeserLaci(k))
    .sort((a, b) => new Date(b.tanggal) - new Date(a.tanggal))
    .map(k => ({ tipe: k.tipe, jumlah: k.jumlah, keterangan: k.keterangan || '-', waktu: fmtDate(k.tanggal) }));
  const digitalMasuk = r.digitalMasuk, digitalKeluar = r.digitalKeluar, digitalNet = r.digitalNet;
  const ctx = {
    bizName: (SETTINGS && SETTINGS.bizName) || 'Kasir Solo - Rosok',
    alamat: [SETTINGS.alamatDetail, SETTINGS.bizDesa, SETTINGS.bizKecamatan, SETTINGS.bizKabkota, SETTINGS.bizProvinsi].filter(Boolean).join(', '),
    tgl: fmtDate(shift.waktuBuka),
    status: 'Shift ditutup',
    summary: [
      { label: 'Modal awal', value: fmtRupiah(shift.modalAwal || 0) },
      { label: 'Kas masuk (' + r.jumlahTransaksi + ' transaksi)', value: '+' + fmtRupiah(r.masuk) },
      { label: 'Kas keluar', value: '−' + fmtRupiah(r.keluar) },
      { label: 'Kas sistem akhir', value: fmtRupiah(sistem) },
      { label: 'Kas fisik dihitung', value: fmtRupiah(fisik) },
      { label: 'Selisih', value: (selisih > 0 ? '+' : '') + fmtRupiah(selisih) },
      ...( (digitalMasuk || digitalKeluar) ? [
        { label: 'Dompet digital masuk (transfer)', value: '+' + fmtRupiah(digitalMasuk) },
        { label: 'Dompet digital keluar (transfer)', value: '−' + fmtRupiah(digitalKeluar) },
  ...
  try { const { printKasShift } = await import('./printer.js'); await printKasShift(ctx); }
  catch(e){ console.warn('[KAS] cetak shift tertutup gagal:', e?.message || e); toast('Gagal mencetak'); }
}
...
window._ksr_toggleKasMutasi = toggleKasMutasi;
window._ksr_cetakKasShift = cetakKasShift;
