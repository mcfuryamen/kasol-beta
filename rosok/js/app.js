...
import { setLaporanPeriode, renderLaporan, openLunasi } from './laporan.js';
import { checkLicenseGate, updateTrialChip, renderLicenseInfoCard, openLicenseSheet, setLicenseRefs } from './license.js';
...
import { connectBTPrinter, disconnectBTPrinter, testPrint, restorePrinterStatus, printNota } from './printer.js';
import { maybeOfferCloudRestore } from './backup.js'; // static (pola kaki5 app.js): window.exportData dll dari backup.js terpasang sejak load — dulu hanya dynamic-import 4 dtk, tombol Data & Cadangan mati di jendela itu
...
async function initApp() {
  ...
  setTimeout(() => { try { maybeOfferCloudRestore(); } catch(_) {} }, 4000);
  markReady();
...
window._ksr_printNota = printNota;
...
// ── Boot splash in-app (pemilik 2026-09-15) ──
// Gradasi oranye + logo bulat saat aplikasi dibuka; pudar begitu initApp
// selesai. Safety timeout menjamin splash tak pernah menjebak user walau boot
// error, dan auto-hide saat tab kembali terlihat.
function hideBootSplash(){
  const s = document.getElementById('bootSplash');
  if(!s || s.classList.contains('gone')) return;
  // Minimum tampil ~0,9 dtk biar tidak cuma "kedip" saat boot sedang cepat.
  const wait = Math.max(0, 900 - (Date.now() - _splashStart));
  setTimeout(()=>{
...
// Gradasi memudar masuk ~0,25 dtk setelah frame solid tampil → sambungan mulus
// dari splash native (solid) ke splash in-app (gradasi).
setTimeout(()=>{ const s=document.getElementById('bootSplash'); if(s) s.classList.add('grad-on'); }, 250);
document.addEventListener('visibilitychange', () => { if(document.visibilityState === 'visible') setTimeout(hideBootSplash, 200); });

window.addEventListener('load', async () => { await initApp(); try{ updateKasBarButtons(); }catch(e){} hideBootSplash(); });
