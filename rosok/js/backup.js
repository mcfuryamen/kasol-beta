...
import { toast, showLoading, hideLoading, getSetting, setSetting, openOverlay, closeSheet, fmtDate } from './utils.js';
import { showConfirm } from './confirm.js';
import { SETTINGS, openShiftCache } from './app-state.js';
import { hmacSignature, isLicensed, ensureUnitId, getLegacyV4DeviceCodes } from './license.js';
...
async function verifyBackupSignature(data, expectedSig){
  ...
  // Masa tenggang identitas (adopsi kaki5 verifyBackupSignature): file lama
  // ditandatangani unit_id era sebelumnya — jangan tolak cadangan sah.
  // Kandidat: unit_id asal re-anchor terakhir, unit era deviceId-acak
  // (legacyDeviceCode tersimpan), dan unit era fingerprint V4 (deterministik,
  // dihitung ulang dari hardware — dua rasa digest × dua orientasi layar).
  try {
    const payload = JSON.stringify(data);
    const candidates = new Set();
    const ra = await getSetting('unitReanchor', null);
    if (ra && ra.from) candidates.add(ra.from);
    const di = await getSetting('deviceIdentity', null) || {};
    if (di.legacyDeviceCode) candidates.add('KSR-' + di.legacyDeviceCode);
    for (const v4 of await getLegacyV4DeviceCodes()) candidates.add('KSR-' + v4);
    for (const unit of candidates) {
      const old = await hmacSignature(unit + payload);
...
async function cloudCtx(){
  // AWAIT WAJIB: isLicensed() async → tanpa await selalu truthy (Promise) →
  // 🔒 pengunci lisensi tidak pernah aktif (bug dilaporkan pemilik 2026-09-14).
  if (!(await isLicensed())) return { err: 'locked' };
...
export async function cloudRestoreLatest(){
  ...
  try {
    ...
    // Tanggal cadangan di konfirmasi (paritas kaki5) — dari stempel waktu
    // save terakhir; cadangan-latest.json hanya satu slot jadi stempel ini
    // adalah umur isi cadangan.
    const lastAt = await getSetting('lastCloudBackupAt', null);
    const tglLbl = lastAt ? ` (${fmtDate(lastAt)})` : '';
    if (!(await showConfirm({ icon:'☁️', text:`Pulihkan cadangan cloud${tglLbl}? Semua data saat ini akan diganti dengan isi cadangan. Lanjutkan?`, okLabel:'Ya, Pulihkan' }))) return;
    showLoading('Memulihkan data...');
    try {
      await applyBackupData(data);
      hideLoading();
...
function cloudErr(kind){
  const msg = {
    locked: '🔒 Fitur ini khusus lisensi aktif — beli lisensi untuk membuka 💳',
  ...
  toast(msg);
  // Paritas kaki5 guardLicensedThen (permintaan pemilik 2026-09-14): klik fitur
  // terkunci HARUS menjawab — selain toast, langsung buka sheet Beli Lisensi.
  if (kind === 'locked' && typeof window._ksr_openPurchaseSheet === 'function') {
    try { window._ksr_openPurchaseSheet(); } catch (_) {}
  }
}
