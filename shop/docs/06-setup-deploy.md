...
Panduan lengkap development dan deployment shop.
...
3. Import repository GitHub yang berisi folder `shop/`
4. Atur:
...
Hubungkan project `shop` ke repo **LIVE `kasol`** (URL `shop-kasol.vercel.app`) dengan **Root Directory = `shop/`** (Vercel → Project → Settings → Git). **GitHub Actions tidak dipakai** (workflow sudah dihapus). Rilis mengalir 2 mirror (folder kerja **tidak** push langsung ke GitHub) — via `push-beta.ps1` → GitHub BETA main, lalu stabil → `push-live.ps1` → GitHub LIVE main.
...
Tidak ada GitHub Actions secrets. Vercel environment variables (mis. `SUPABASE_URL`, `SUPABASE_ANON_KEY` untuk shop) diatur di dashboard Vercel → Project → Settings → Environment Variables. `.env.local` hanya untuk dev lokal (gitignored).
...
### Katalog tidak muncul di shop

Cek apakah Control Center sudah pernah menyimpan data ke localStorage.
Jika kosong, shop akan menampilkan `DEFAULT_CATALOG` dari source code.
