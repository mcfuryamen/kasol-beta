...
Shop ini memiliki 4 tujuan utama:
...
Admin dapat menambah, mengubah, atau menghapus aplikasi melalui Control Center.
Perubahan otomatis muncul saat shop dimuat ulang.

### Form Trial → Lead
Ketika pengunjung mengisi form trial:
1. Data disimpan ke `localStorage` key `kasirsolo:leads`
2. Form reset + pesan sukses muncul
3. Data otomatis tersedia di Control Center (tab Leads)
...
Shop membaca 4 key dari `localStorage`:

| Key | Isi | Sumber Penulisan |
|-----|-----|-----------------|
| `kasirsolo:catalog` | Daftar aplikasi (nama, harga, kategori, dll) | Control Center |
| `kasirsolo:settings` | Kontak, alamat, statistik hero | Control Center |
...
*Shop — KASIRSOLO by PT Mesin Kasir Solo*
