...
│   └─────────────────┘                                                  │
│                                                                         │
│   CATATAN: Shop hanya BISA menulis leads & stats.              │
...
|-----|------|------------|-------------|
| `kasirsolo:catalog` | Array of objects | Shop | Control Center |
| `kasirsolo:settings` | Object | Shop | Control Center |
| `kasirsolo:leads` | Array of objects | Control Center | Shop (form) |
| `kasirsolo:stats` | Object `{visits: N}` | Control Center | Shop (setiap load) |
...
Shop di-deploy otomatis ke **Vercel** lewat **git integration (auto-detect)** — project `shop` dengan **Root Directory = `shop/`**. **GitHub Actions tidak dipakai** (semua workflow sudah dihapus). Push ke branch utama → Vercel auto-deploy.
...
Shop akan menggunakan Supabase JS client untuk membaca/menulis data.
Admin dashboard akan menjadi satu-satunya writer untuk catalog & settings.

---

*Arsitektur Shop — KASIRSOLO*
