...
```
┌─────────────────────────────────────────────────────────────────────────┐
│  LAPISAN A — META / CRM  (Supabase SEKARANG, via repo control/)           │
...
Diakses & dikelola melalui **repo `control/`** (Control Center). Ini adalah *CRM
...
> ⚠️ **Catatan status:** `control/` kini **sudah terhubung Supabase** (katalog CRUD +
...
> (`verifyAndAssignSerial`), `control/js/clients.js` (`reassignClientUnit`),
